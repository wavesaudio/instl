...
class InstallItem(object):
    ...
    def get_recursive_depends(self, items_map, out_set, orphan_set):
        IID = self.var_list.resolve_var("iid_iid")
    ...
    def repr_for_yaml(self):
        retVal = OrderedDict()
        retVal['name'] = self.var_list.resolve_var("iid_name")
        if "iid_guid" in self.var_list:
            retVal['guid'] = self.var_list.resolve_var("iid_guid")
        if "iid_remark" in self.var_list:
            retVal['remark'] = self.var_list.resolve_var("iid_remark")
        if self.inherit:
    ...
    def merge_from_another_InstallItem(self, otherInstallItem):
        ...
        if "iid_remark" not in self.var_list:
            the_remark = self.var_list.resolve_var("iid_name")
        the_remark += ", "+otherInstallItem.var_list.resolve_var("iid_name")
    ...
    def resolve_inheritance(self, InstallItemsDict):
        if not self.__resolved_inherit:
            ...
            for ancestor in self.inherit_list():
                ...
                the_remark = ", ".join( (self.var_list.resolve_var("iid_remark"), ancestor_item.var_list.resolve_var("iid_name"), ancestor_item.var_list.resolve_var("iid_remark")) )
                self.var_list.set_var("iid_remark").append(the_remark)
...
def guid_list(items_map):
    retVal = unique_list()
    retVal.extend(filter(bool, [install_def.var_list.resolve_var("iid_guid") for install_def in items_map.values()]))
    return retVal
...
def iids_from_guid(items_map, guid):
    ...
    for iid, install_def in items_map.iteritems():
        if install_def.var_list.resolve_var("iid_guid") == guid:
            retVal.append(iid)
    return retVal

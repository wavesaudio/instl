...
def get_path_to_instl_app(launch_file):
    ...
    if getattr(sys, 'frozen', False):
        ...
        application_path = Path(launch_file)
    return application_path


def get_instl_launch_command(launch_file):
    """
    @return: returns the path to this
    """
    launch_command = None
...
def get_data_folder(launch_file):
    application_path = get_path_to_instl_app(launch_file)
    data_folder = None
    if getattr(sys, 'frozen', False):
        data_folder: Path = application_path.parent
        defaults_folder: Path = data_folder.joinpath('defaults')
        if not defaults_folder.is_dir():
            data_folder = Path(getattr(sys, '_MEIPASS', None))
    elif launch_file:
        data_folder = Path(launch_file).parent
    return data_folder
...
def instl_own_main(launch_file, argv):
    ...
    with InvocationReporter(argv):
        ...
        initial_vars = {"__INSTL_EXE_PATH__": get_path_to_instl_app(launch_file),
                        "__CURR_WORKING_DIR__": os.getcwd(),  # the working directory when instl was launched
                        "__INSTL_LAUNCH_COMMAND__": get_instl_launch_command(launch_file),
                        "__INSTL_DATA_FOLDER__": get_data_folder(launch_file),

...
class InstlInstanceSync_s3url(InstlInstanceSync):
    """  Class to create sync instruction using static links.
    """

    def __init__(self, instlObj):
        super(InstlInstanceSync_s3url, self).__init__(instlObj)

    def init_sync_vars(self):
        super(InstlInstanceSync_s3url, self).init_sync_vars()
        self.local_sync_dir = var_list.resolve("$(LOCAL_REPO_SYNC_DIR)")

    def create_sync_instructions(self, installState):
        super(InstlInstanceSync_s3url, self).create_sync_instructions(installState)

...
class SVNTree(svnItem.SVNTopItem):
    ...
    def read_from_yaml(self, rfd):
        try:
            ...
            raise instlException.InstlException(" ".join(("YAML error while reading file", "'" + file_path + "':\n", str(ye))), ye)
        except IOError as ioe:
            raise instlException.InstlException(" ".join(("Failed to read file", "'" + file_path + "'", ":")), ioe)

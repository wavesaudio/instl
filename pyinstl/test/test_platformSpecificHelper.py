...
def prepare_test_folder(folder_name):
    ...
    utils.safe_makedirs(test_folder)
    return test_folder
...
class TestPlatformSpecificHelper(unittest.TestCase):
    ...
    def check_indoes_Equal(self, left_dir, right_dir, test_name, report=None):
        ...
        if not retVal:
            print(test_name)
            col_format = utils.gen_col_format(utils.max_widths(report))
    ...
    def test_copy_dir_to_dir_with_hard_links(self):
        ...
        originals_folder = utils.safe_makedirs(os.path.join(test_folder, "originals"))
        originals_file_paths = create_random_files(originals_folder)
        sub_originals_folder =  utils.safe_makedirs(os.path.join(test_folder, "originals", "sub-originals"))
        originals_file_paths.extend(create_random_files(sub_originals_folder))
         utils.safe_makedirs(os.path.join(test_folder, "originals", "sub-sub-originals-empty"))
        sub_sub_originals_folder_full =  utils.safe_makedirs(os.path.join(test_folder, "originals", "sub-sub-originals-full"))
        originals_file_paths.extend(create_random_files(sub_sub_originals_folder_full))

        hard_links_folder =  utils.safe_makedirs(os.path.join(test_folder, "hard-links"))
    ...
    def test_copy_dir_to_dir_no_hard_links(self):
        ...
        copies_folder =  utils.safe_makedirs(os.path.join(test_folder, "copies"))
    ...
    def test_copy_dir_contents_to_dir_with_hard_links(self):
        ...
        hard_links_folder =  utils.safe_makedirs(os.path.join(test_folder, "hard_links"))

        # copy without hard-links, file contents should be the same. files' inodes should be different.
        copy_command = self.ps_helper.copy_tool.copy_dir_contents_to_dir(originals_folder, hard_links_folder,
                                                                         link_dest=True)
        subprocess.check_output(copy_command, stdin=None, stderr=None, shell=True, universal_newlines=False)
    ...
    def test_copy_dir_contents_to_dir_no_hard_links(self):
        ...
        # copy without hard-links, file contents should be the same. files' inodes should be different.
        copy_command = self.ps_helper.copy_tool.copy_dir_contents_to_dir(originals_folder, copies_folder,
                                                                         link_dest=False)
        subprocess.check_output(copy_command, stdin=None, stderr=None, shell=True, universal_newlines=False)
    ...
    def test_files_to_dir_copy_with_hard_links(self):
        ...
        copy_command = self.ps_helper.copy_tool.copy_dir_files_to_dir(originals_folder, hard_links_folder,
                                                                      link_dest=True)
        subprocess.check_output(copy_command, stdin=None, stderr=None, shell=True, universal_newlines=False)
    ...
    def test_file_to_dir_copy_no_hard_link(self):
        ...
        self.assertTrue(filecmp.cmp(file_to_copy, copied_file, shallow=False),
                        "{copied_file} file is different from expected {file_to_copy}".format(**locals()))
        self.assertTrue(self.check_indoes_NotEqual(originals_folder, copies_folder, inspect.stack()[0][3]))
    ...
    def test_file_to_dir_copy_with_hard_link(self):
        ...
        originals_folder =  utils.safe_makedirs(os.path.join(test_folder, "originals"))
        hard_link_folder =  utils.safe_makedirs(os.path.join(test_folder, "hard-link"))
        ...
        self.assertTrue(filecmp.cmp(file_to_copy, hard_link_file, shallow=False),
                        "{hard_link_file} file is different from expected {file_to_copy}".format(**locals()))

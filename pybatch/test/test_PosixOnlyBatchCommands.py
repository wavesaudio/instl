...
class TestPythonBatchPosix(unittest.TestCase):
    ...
    def test_CreateSymlink(self):
        ...
        self.assertFalse(os.path.islink(a_file_to_symlink_to_abs),
                         f"CreateSymlink {a_file_to_symlink_to_abs} should be a file not a symlink")
        self.assertFalse(os.path.islink(a_file_to_symlink_to_relative),
                         f"CreateSymlink {a_file_to_symlink_to_relative} should be a file not a symlink")
        self.assertFalse(os.path.islink(a_file_to_symlink_to_relative),
                         f"CreateSymlink {a_file_to_symlink_to_relative} should be a file not a symlink")
        self.assertFalse(os.path.islink(a_folder_to_symlink_to_relative),
                         f"CreateSymlink {a_folder_to_symlink_to_relative} should be a file not a symlink")

        self.assertTrue(os.path.islink(symlink_to_a_file_abs), f"CreateSymlink {symlink_to_a_file_abs} should be a symlink")
        self.assertTrue(os.path.islink(symlink_to_a_file_relative),
                        f"CreateSymlink {symlink_to_a_file_relative} should be a symlink")
        self.assertTrue(os.path.islink(symlink_to_a_folder_abs),
                        f"CreateSymlink {symlink_to_a_folder_abs} should be a symlink")
        self.assertTrue(os.path.islink(symlink_to_a_folder_relative),
                        f"CreateSymlink {symlink_to_a_folder_relative} should be a symlink")

        # check the absolute symlinks
        a_file_original_from_symlink_abs = os.readlink(symlink_to_a_file_abs)
        self.assertTrue(a_file_to_symlink_to_abs.samefile(a_file_original_from_symlink_abs),
                        f"symlink resolved to {a_file_original_from_symlink_abs} not to {a_file_to_symlink_to_abs} as expected")
        a_folder_original_from_symlink_abs = os.readlink(symlink_to_a_folder_abs)
        self.assertTrue(a_folder_to_symlink_to_abs.samefile(a_folder_original_from_symlink_abs),
                        f"symlink resolved to {a_folder_original_from_symlink_abs} not to {a_folder_to_symlink_to_abs} as expected")
        ...
        a_file_original_from_symlink_relative = self.pbt.path_inside_test_folder(os.readlink(symlink_to_a_file_relative),
                                                                                 assert_not_exist=False)
        self.assertTrue(a_file_to_symlink_to_relative.samefile(a_file_original_from_symlink_relative),
                        f"symlink resolved to {a_file_original_from_symlink_relative} not to {a_file_to_symlink_to_abs} as expected")
        a_folder_original_from_symlink_relative = self.pbt.path_inside_test_folder(
            os.readlink(symlink_to_a_folder_relative), assert_not_exist=False)
        self.assertTrue(a_folder_to_symlink_to_relative.samefile(a_folder_original_from_symlink_relative),
                        f"symlink resolved to {a_folder_original_from_symlink_relative} not to {a_folder_to_symlink_to_abs} as expected")
        ...
        self.assertFalse(symlink_to_a_folder_relative.is_symlink(),
                         f"{symlink_to_a_folder_relative} should have been removed")
    ...
    def test_SymlinkToSymlinkFileAndBack(self):
        ...
        """
        SymlinkTestData = namedtuple('SymlinkTestData',
                                     ['original_to_symlink', 'symlink_to_a_original', 'symlink_file_of_original',
                                      'relative_symlink_to_a_original', 'symlink_file_of_relative'])
        ...
        def create_symlink_test_data(name):
            ...
            return SymlinkTestData(original_to_symlink, symlink_to_a_original, symlink_file_of_original,
                                   relative_symlink_to_a_original, symlink_file_of_relative)
        ...
        for test_data in file_symlink_test_data, folder_symlink_test_data:
            with self.pbt.batch_accum.sub_accum(
                    CdStage(test_data.original_to_symlink.name, self.pbt.test_folder)) as symlink_test_accum:
                symlink_test_accum += CreateSymlink(test_data.symlink_to_a_original,
                                                    test_data.original_to_symlink)  # symlink with full path
                symlink_test_accum += CreateSymlink(test_data.relative_symlink_to_a_original,
                                                    test_data.original_to_symlink.name)  # symlink with relative path
        ...
        for test_data in file_symlink_test_data, folder_symlink_test_data:
            self.assertFalse(os.path.islink(test_data[0]),
                             f"SymlinkToSymlinkFile {test_data.original_to_symlink} should be a file not a symlink")
            self.assertFalse(test_data.symlink_to_a_original.exists(),
                             f"SymlinkToSymlinkFile {test_data.symlink_to_a_original} should have been erased")
            self.assertFalse(test_data.relative_symlink_to_a_original.exists(),
                             f"SymlinkToSymlinkFile {test_data.relative_symlink_to_a_original} should have been erased")
            self.assertTrue(test_data.symlink_file_of_original.is_file(),
                            f"SymlinkToSymlinkFile {test_data.symlink_file_of_original} should have been replaced by .symlink file")
            self.assertTrue(test_data.symlink_file_of_relative.is_file(),
                            f"SymlinkToSymlinkFile {test_data.symlink_file_of_relative} should be replaced by .symlink file")
        ...
        for test_data in file_symlink_test_data, folder_symlink_test_data:
            ...
            self.assertTrue(test_data.symlink_to_a_original.is_symlink(),
                            f"SymlinkToSymlinkFile {test_data.symlink_to_a_original} should have been created")
            self.assertTrue(test_data.relative_symlink_to_a_original.is_symlink(),
                            f"SymlinkToSymlinkFile {test_data.relative_symlink_to_a_original} should have been created")

            # check that the absolute and relative symlinks files have been removed
            self.assertFalse(test_data.symlink_file_of_original.exists(),
                             f"SymlinkToSymlinkFile {test_data.symlink_file_of_original} should have been erased")
            self.assertFalse(test_data.symlink_file_of_relative.exists(),
                             f"SymlinkToSymlinkFile {test_data.symlink_file_of_relative} should have been erased")
            ...
            self.assertTrue(test_data.original_to_symlink.samefile(an_original_from_symlink),
                            f"symlink resolved to {an_original_from_symlink} not to {test_data.symlink_to_a_original} as expected")
            os.chdir(self.pbt.test_folder)  # so relative resolve of symlink will work
            self.assertTrue(test_data.original_to_symlink.samefile(an_original_from_relative_symlink),
                            f"symlink resolved to {an_original_from_relative_symlink} not to {test_data.symlink_to_a_original} as expected")

...
- `CurlTransfer.run()` — Windows long-path workaround via `win32api.GetShortPathName`; obtains the pause channel via module-level `get_control_channel()`; then delegates to `_run_config_with_recovery` (the pause/offline-hold/backoff re-run loop) and, on exit 0, `_reconcile_missing_outputs` (re-download entries whose expected `output` file is missing — curl `--parallel` can exit 0 with permanently failed transfers); finally runs the exit-33 fallback when `can_run_fallback(...)`. Returns curl's final code. See the Download Subsystem section for the full offline-hold / reconciliation / stall-watchdog method inventory and the `DOWNLOAD_CLIENT_HANDLES_BACKEND_HOLD` event gate.
...
if return_code == 0:
    return_code = _reconcile_missing_outputs(pause_check, channel)   # curl --parallel can exit 0 with failed transfers

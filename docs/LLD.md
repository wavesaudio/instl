...
Consumed by `instlInstanceSync_url.py:162`, `info_mapBatchCommands.py:507`, the curl drivers (`ParallelRun._control_channel` in `subprocessBatchCommands.py`, `downloadTransfer.get_control_channel`), and `downloadRetry.sleep_backoff`.
...
The actual curl execution engine invoked by the `curlHelper`-generated commands. `CurlWithInternalParallel` (`subprocessBatchCommands.py:669`) is only the pybatch command — 4 methods (`__init__`, `progress_msg_self`, `repr_own_args`, `__call__`) that carry the serialized numbers and hand them to a `CurlTransfer`, which owns all of the orchestration below.
- `ParallelRun._control_channel()` (line 324): a seam over `downloadTransfer.get_control_channel()` (lazy `get_global_channel()`, returning `None` if unavailable, so a non-curl `ParallelRun`, e.g. a copy, is never paused; the method stays as the hook tests override).
- `ParallelRun._run_with_pause_and_offline_hold(commands)` (line 338): the pause/offline-hold/network-retry/exit-33-fallback run loop (see Algorithms). `network_retry_budget = 12`.
- Shared by both drivers as module-level functions in `downloadTransfer`: `is_network_error(exit_code)` (checks `utils.NETWORK_ERROR_CURL_EXIT_CODES`), `can_run_fallback(fallback_config_file, exit_code, fallback_exit_codes)` (gates on a fallback config being set and the exit code being in `fallback_exit_codes`, `(33,)`), and `get_control_channel()`. `_run_fallback_after_curl_range_failure(exit_code)` exists on `ParallelRun` (line 417) **and** on `CurlTransfer` and is deliberately *not* shared: one re-runs a parallel-run command file through `utils.run_processes_in_parallel`, the other hands curl a single `--config` file.
...
#### 2.3 `RunProcessBase` (`pybatch/subprocessBatchCommands.py:37`)
...
- `get_run_args(run_args)` (`:61`, abstract) — subclasses build the argv list.
- `__call__` (`:64`) — runs `subprocess.run(check=False, stderr=PIPE, shell=self.shell, bufsize=0)`; handles `detach` (Popen), `script`/`shell` single-string modes, `out_file`/`capture_stdout`. **stderr policy** (`:114-131`): if the process wrote to stderr and `stderr_means_err` and exit code was 0, the code is forced to **123**; `ignore_all_errors` forces returncode 0. Calls `handle_completed_process()`.
- `should_ignore__exit__exception` (`:148`) — additionally ignores `CalledProcessError` whose returncode is in `ignore_specific_exit_codes`.

`CUrl` (`:156`) builds a single-file curl argv (`--fail --raw --silent --show-error --connect-timeout --max-time --retry --retry-delay -o trg src`).

#### 2.4 `CurlWithInternalParallel` (`pybatch/subprocessBatchCommands.py:669`) and `CurlTransfer` (`pyinstl/downloadTransfer.py`)
...
`ParallelRun._run_with_pause_and_offline_hold` (`subprocessBatchCommands.py:338`) implements a near-identical loop for the multi-process parallel runner; the pause check is only wired when the commands are curl (`_is_curl_command`, `:414`) so a pause can't affect e.g. a copy parallel-run.
...
2. **Class-level mutable global state** (`baseClasses.py:47-57`) — `stage_stack`, `running_progress`, `total_progress`, etc. are process-global and mutated during enter/exit, which breaks `RunInThread` (subprocessBatchCommands.py:481) reentrancy. Move run-scoped state into an explicit run/session object (or thread-local) passed down the tree.

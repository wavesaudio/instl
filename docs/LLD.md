...
- `calc_size_of_file_item()` — wtar size scaled by `wtar_ratio`; accumulates `bytes_to_copy`.
...
- `DownloadStateStore`: rooted at `<bookkeeping_dir>/download-state`; `session_path()` → `session.json`, `file_path(file_id)` → `files/{file_id}.json`. `from_bookkeeping_dir(...)`, `load/save_session`, `load/save_file`.
...
- `PythonBatchRuntime(name)` (`call__call__=False`, `is_context_manager=True`) — wraps the whole run. `__exit__`: on exception calls `log_error`; logs total run time; pops the stage. `log_error` finds the originating command via `exc_val.raising_obj` and dumps `error_dict(...)` as JSON to the log.
...
previously_downloaded_files = _files_delivered_by_earlier_chunks() or planned base   # measured, not planned
...
- **Bytes**: cumulative — with curl `continue-at`, a run's `Dled` is only the *new* bytes, so `cumulative = _bytes_baseline + current_dled`, clamped via `_bytes_high_water`, capped at total. When the run ends/pauses, the run's final `Dled` is folded into `_bytes_baseline`.

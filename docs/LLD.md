...
> This document names symbols (classes, methods, module-level functions) rather than line numbers,
> because line numbers rot on every edit. A typed `config_vars` accessor seam lives at
> `configVar/accessors.py`. See `docs/TESTING.md` for what is and is not covered by tests.
...
#### `InvocationReporter(PythonBatchRuntime)` — `pyinstl/instl_main.py`
...
#### `OptionToConfigVar` — `pyinstl/cmdOptions.py`
...
#### `CommandLineOptions` — `pyinstl/cmdOptions.py`
...
#### `prepare_args_parser(in_command)` — `pyinstl/cmdOptions.py`
...
#### `read_command_line_options(name_space_obj, arg_list=None)` — `pyinstl/cmdOptions.py`
...
#### Path / launch resolvers — `pyinstl/instl_main.py`
...
#### `fix_ssl_paths()` — `pyinstl/instl_main.py`
...
#### `_start_control_channel_if_needed(main_command)` / `_CONTROL_CHANNEL_COMMANDS` — `pyinstl/instl_main.py`
...
Raised across the codebase; caught explicitly at the interactive loop (`instlInstanceBase_interactive.py`) and otherwise handled by `PythonBatchRuntime.__exit__`.

### Public interfaces / contracts

- `instl_own_main(argv) -> None` — sole boot entry; called by the `instl` launcher and reused as a `multiprocessing.Process` target by `pyinstl/instlAdmin.py` (up2s3 recursive invocation).
...
- `instance.init_from_cmd_line_options(options)` — the contract every command instance implements (`pyinstl/instlInstanceBase.py`); called by `instl_main` (and `CommandListRunner`) after construction. It copies remaining namespace fields (`which_revision`, `subject`, `--define X=y` pairs) into `config_vars` and computes the default out file.
...
### `initial_vars` (selected keys — `instl_main.py`)
...
- **OS-name derivation** (`instl_main.py`): `os_family_name = current_os_names[0]`; `os_second_name` is initialized to `current_os_names[0]` and only overwritten with `current_os_names[1]` if `len > 1`.
...
6. **Catalog + argparse wiring intertwined** — `prepare_args_parser` (`cmdOptions.py`) mixes the command→mode/options/help data with imperative argparse construction; the cascade of `if in_command not in all_command_details` is subtle. Direction: move the catalog to a declarative table/dataclasses and build groups from a token→builder map.
7. **Inconsistent `options` token syntax** — `cmdOptions.py`: `read-info-map` uses the string `'in+'` (not a tuple, so membership works only by substring accident) and `short-index` uses a set literal `{'in','out'}` while every other entry uses a tuple. Normalize all to tuples of tokens.
8. **Duplicated random-id and UTF-8 reopen logic** — `InvocationReporter.random_invocation_name` (`instl_main.py`) vs `__INVOCATION_RANDOM_ID__` generate two independent 16-char ids; the two stdout/stderr reopen blocks in `instl` are near-identical. Factor `make_random_id()` and `reopen_utf8(stream)` helpers and decide whether one id suffices.
...
#### `IndexYamlReaderBase` (mixin) — `pyinstl/instlInstanceBase.py`
...
- `init_specific_doc_readers(self)` — registers `!require → self.read_require`, `!index → self.read_index`, and if `TARGET_OS` is set, an OS-specific tag `!index_$(TARGET_OS)` (e.g. `!index_Mac`). **Invariant/edge case**: raises `AssertionError` if both `!index_Mac` and `!index_Win` end up registered simultaneously.
...
#### `InstlInstanceBase` — `pyinstl/instlInstanceBase.py`

`class InstlInstanceBase(IndexYamlReaderBase, metaclass=abc.ABCMeta)`.

**Class attribute**
- `commands_that_need_to_refresh_db_file` — list of command names that force a fresh DB file: `copy, sync, synccopy, uninstall, remove, doit, read-yaml, translate-guids, verify-repo, depend, fix-props, up2s3, activate-repo-rev, short-index, up-short-index, report-versions`.

**Key instance attributes** (set in `__init__`)
...
- `progress(self, *messages)` — no-op unless `total_self_progress > 0`. Increments `internal_progress`; when it reaches the threshold, multiplies the threshold by 5 (a growing-denominator progress heuristic) and logs `Progress: N of M; <messages>`.
- `init_specific_doc_readers(self)` — extends the mixin's readers. Pops `__no_tag__`/`__unknown_tag__`. Registers `!define` and the deprecated `!define_const` (both → `read_defines`, i.e. const is treated as non-const). Builds the `ACCEPTABLE_YAML_DOC_TAGS` set, appending `define_Compiled` or `define_Uncompiled` based on `__INSTL_COMPILED__`, then registers `!define_if_not_exist*` → `read_defines_if_not_exist` and other `define*` → `read_defines`.
- `init_default_vars(self, initial_vars)` — registers the dynamic var `__NOW__` (callback returns `str(datetime.datetime.fromtimestamp(time.time()))`), applies `initial_vars`, sets value-set callbacks for `ACTING_UID`/`ACTING_GID` (`utils.set_active_user_or_group_config_var_callback`), then reads `defaults/main.yaml` (mandatory: `ignore_if_not_exist=False`) and `defaults/compile-info.yaml`. If `__COMPILATION_TIME__` is absent it sets `"unknown compilation time"` (compiled) or `"(not compiled)"`. Finally `read_user_config()`.
- `read_defaults_file(self, file_name, allow_reading_of_internal_vars=True, ignore_if_not_exist=True)` — reads `__INSTL_DEFAULTS_FOLDER__/<file_name>.yaml`.
- `read_user_config(self)` — reads `__USER_CONFIG_FILE_PATH__` (ignored if missing).
- `check_prerequisite_var_existence(self, prerequisite_vars)` — raises `ValueError` listing any missing required vars.
- `init_from_cmd_line_options(self, cmd_line_options_obj)` — maps CLI options into config_vars. Sets `the_command`/`fixed_command` from `__MAIN_COMMAND__`. Computes `db_need_refresh = mode == "interactive" or the_command in commands_that_need_to_refresh_db_file` and calls `DBManager.set_refresh_db_file(...)`. Sets `__HELP_SUBJECT__`, `__WHICH_REVISION__`, parses `--define key=value` (comma-split, then `=` split), then `get_default_out_file()`. **Edge case**: `--define` parsing does `definition.split("=")` which breaks if a value itself contains `=`.
- `close(self)` — `del self.info_map_table; del self.items_table; del self.db; config_vars.print_statistics()`.
- `get_default_out_file(self)` — if `__MAIN_OUT_FILE__` unset but `__MAIN_INPUT_FILE__` set, derives `$(__MAIN_INPUT_FILE__)-$(__MAIN_COMMAND__).$(BATCH_EXT)`.
- `read_require(self, a_node, *args, **kwargs)` — **redundant override**, identical to the parent mixin's method.
- `write_require_file(self, file_path, require_dict)` — serializes a `!define` doc (`REQUIRE_REPO_REV`, `REQUIRE_REPO_NAME`, `REQUIRE_SYNC_BASE_URL`, `REQUIRE_S3_BUCKET_NAME`) followed by a `!require` doc via `aYaml.writeAsYaml`.
- `resolve_defined_paths(self)` — adds `SEARCH_PATHS` to the searcher and resolves each var listed in `PATHS_TO_RESOLVE` via `path_searcher.find_file(..., return_original_if_not_found=True)`.
- `read_include_node(self, i_node, *args, **kwargs)` — handles the `!include` tag (see algorithm below).
- `create_variables_assignment(self, in_batch_accum)` — switches accum to the `assign` section; builds a case-insensitive regex from `DONT_WRITE_CONFIG_VARS` (plus `os.environ` keys unless `WRITE_CONFIG_VARS_READ_FROM_ENVIRON_TO_BATCH_FILE` is truthy) and emits `ConfigVarAssign(identifier, *values)` for every non-excluded config var.
- `init_python_batch(self, in_batch_accum)` — switches to `begin` section; emits `PythonDoSomething` bootstrap lines for `RsyncClone` global ignore/no-hard-link/no-flags/avoid-copy-marker patterns (avoid-copy only when not `update_mode`), `RemoveEmptyFolders` default ignore files, and `log.setLevel(PYTHON_BATCH_LOG_LEVEL or 20)`.
- `calc_user_cache_dir_var(self)` — platform branch on `__CURRENT_OS__`; computes `USER_CACHE_DIR` via `appdirs.user_cache_dir`. **Mac and Linux branches are identical**; Win passes app and author separately; unknown OS raises `RuntimeError`.
- `get_aux_cache_dir(self, make_dir=True)` — returns `LOCAL_REPO_REV_BOOKKEEPING_DIR` if fully resolved (i.e. S3_BUCKET_NAME/REPO_NAME/REPO_REV are known), otherwise `USER_CACHE_DIR/cache`. Optionally creates it via `MakeDir`.
- `get_default_sync_dir(self, continue_dir=None, make_dir=True)` — `USER_CACHE_DIR[/continue_dir]`, optionally created.
- `relative_sync_folder_for_source(self, source)` and `relative_sync_folder_for_source_table(self, adjusted_source, source_type)` — for `!dir`/`!file` return the path with the last segment stripped; for `!dir_cont` return the path as-is; else `ValueError`. These two are near-duplicates differing only in input shape.
- `write_batch_file(self, in_batch_accum, file_name_post_fix="")` — see algorithm below.
- `run_batch_file(self)` — if `out_file_realpath` ends `.py`, `compile(..., optimize=2)` and `exec(py_compiled, globals())` **in-process**; otherwise `subprocess.Popen` the file, print stdout/stderr, raise `SystemExit` on nonzero return code.
- `read_index(self, a_node, *args, **kwargs)` — progress-wrapping override calling the mixin, then logs `REPO_REV`.
- `find_cycles(self)` — lazy-imports `installItemGraph`; builds dependency + inherit graphs and logs any cycles; swallows `ImportError`.
- `needs(self, iid, all_iids_set=None, cache=None)` — recursive transitive-dependency collection (see algorithm).
- `needed_by(self, iid, graph=None)` — lazy-imports `installItemGraph`, returns sorted `find_needed_by` result or `None` on `ImportError`.
- `handle_yaml_read_error(self, **kwargs)` — best-effort logging of YAML parse errors (path, position, permissions, exception); swallows any exception internally.
- `verify_actions(self, problem_messages_by_iid=None)` — activates all OSes, pulls all index actions, and validates each by attempting `EvalShellCommand(action, None, self.python_batch_names, raise_on_error=True)`. Counts `ValueError`s as bad actions and records messages per IID.
- `write_config_vars_to_file(self, path_to_config_vars_file)` — dumps `config_vars.repr_for_yaml()` as a `!define` YAML doc.
- `get_version_str(self, short=False)` — resolves `__INSTL_VERSION_STR_SHORT__` / `__INSTL_VERSION_STR_LONG__`.

#### `check_version_compatibility()` — `pyinstl/instlInstanceBase.py`
...
- `go_interactive(client, admin)` — **monkey-patches** `InstlInstanceBase.create_completion_list = create_completion_list_imp` and `InstlInstanceBase.do_list = do_list_imp` onto the class at runtime, then runs `CMDObj(client, admin).cmdloop()` inside a `with` block; logs any exception.
- `CMDObj(cmd.Cmd, object)` — REPL holding `client_prog_inst` and `admin_prog_inst`, a `restart` flag, and `this_program_name` from `INSTL_EXEC_DISPLAY_NAME`.
  - `__enter__` — readline binds (libedit vs GNU tab handling, ignore-case, expand-tilde, etc.), creates history dir under `appdirs.user_data_dir`, reads history file (deletes it on read failure), sets prompt, saves CWD.
  - `__exit__` — `compact_history()`, truncates history to 32, writes history file (all best-effort), restores CWD, and `restart_program()` if `restart` set.
  - `onecmd` — wraps the base dispatcher; catches `InstlException` (prints `.message` + original traceback) and any other exception (prints traceback).
  - Command handlers: `do_apropos`, `do_list`, `do_statistics`, `do_set`/`do_del` (mutate config_vars), `do_read`/`do_readinfo`/`do_listinfo` (read index/info-map files into tables), `do_cycles`/`do_depend` (delegate to `find_cycles`/`needs`/`needed_by`), `do_sync`/`do_copy` (set `__MAIN_OUT_FILE__`/`__MAIN_COMMAND__` then call `client_prog_inst.do_command()`), `do_version`, `do_restart`/`do_r`, `do_quit`/`do_q`, `do_hist`, `do_hh` (lazy-imports `help.helpHelper.do_help`), `do_python` (**unguarded `eval`**), `do_resolve` (`config_vars.resolve_str`), `do_which`, `do_stam` (debug: `items_table.iids_from_guids2`). Each has matching `help_*`/`complete_*` where relevant.
  - `emptyline` overridden to return `False` to avoid PyCharm double-execution.
- `do_list_imp(self, what=None, stream=sys.stdout)` — dumps `define`/`index`/`guid` sections (or GUID-matched IIDs via `utils.guid_re`) as YAML using `aYaml.YamlDumpDocWrap`. **Edge case**: when `what is None` it writes `self` then continues with an empty `list_to_do` (the `return` is missing after the `None` write).
- `create_completion_list_imp(self, for_what="all")` — builds tab-completion list from `items_table.get_all_iids()`, `config_vars.keys()`, and guid detail values; swallows exceptions.
- Utilities: `restart_program()` (`os.execl` re-exec), `compact_history()` (dedupes readline history), `insensitive_glob()`, `text_with_color(text, color)` (**no-op stub returning `text` unchanged**). `current_os` is computed at module import via its own `platform.system()` match.

### Key algorithms

**1. `read_include_node` — `!include` handling**
...
**2. `write_batch_file` — batch serialization**
...
**3. `needs` — transitive dependency closure**
```
...
```
**Bug**: `retVal` is a `set` but the missing-dependency branch calls `retVal.append(...)`, which raises `AttributeError`. This branch only fires when a declared dependency is not among the known IIDs.
...
- `!index_Mac` and `!index_Win` cannot both be registered (`AssertionError` in `IndexYamlReaderBase.init_specific_doc_readers`).
...
1. **God-object base class** — `InstlInstanceBase` (whole class) mixes config loading, YAML tag reading, DB lifecycle, path resolution, batch generation, batch execution, dependency queries, action verification, and serialization. All five command subclasses inherit every concern. Extract collaborators: a `PathResolver` (`calc_user_cache_dir_var`/`get_aux_cache_dir`/`get_default_sync_dir`/`relative_sync_folder_*`), a `BatchFileWriter` (`create_variables_assignment`/`init_python_batch`/`write_batch_file`/`run_batch_file`), and a `DependencyAnalyzer` (`needs`/`needed_by`/`find_cycles`/`verify_actions`); compose rather than inherit.
2. **`needs` set/append bug** — in `needs`, `retVal.append(...)` on a `set` raises `AttributeError`. Use `retVal.add(dep + "(missing)")`.
...
4. **Redundant override** — `InstlInstanceBase.read_require` is byte-for-byte identical to the parent mixin's. Delete it; keep only the progress-wrapping `read_index` override.
5. **Runtime monkey-patching** — `go_interactive` assigns `do_list`/`create_completion_list` onto the class at runtime; the methods don't exist until interactive mode runs, defeating static analysis. Define them as real methods on `InstlInstanceBase` or a small `InteractiveMixin`.
6. **`exec`/`eval` on dynamic input** — `run_batch_file` execs the generated `.py` into the host `globals()`; `do_python` is an unguarded `eval`. Prefer running generated scripts uniformly via subprocess (the else-branch already does) and scoping/limiting `do_python`.
7. **Dead/no-op code** — the `if False:` `DoingDecorator` block and its commented decorators; `text_with_color` no-op stub (still called throughout `do_depend`); the large commented-out `complete_listinfo`; empty `help_resolve` body. Remove or implement.
8. **Duplicated platform/OS detection** — collapse the identical Mac/Linux branches in `calc_user_cache_dir_var` and centralize OS detection shared with the interactive module's import-time `current_os`.
9. **Unused local** — `exit_on_errors` computed in `write_batch_file` is never used; either thread it through or remove it.
10. **Fragile `--define` parsing** — `init_from_cmd_line_options` splits on `=` unconditionally, so values containing `=` break. Use `split("=", 1)`.
...
- `create_download_instructions()` — computes already-synced vs to-download counts/bytes, sets `__NUM_FILES_TO_DOWNLOAD__`/`__NUM_BYTES_TO_DOWNLOAD__`, short-circuits when 0 files. Otherwise assembles: `create_sync_folders()` → `PrepareDownloadTempFiles(own_progress_count=0)` → `ReportDownloadStarted(files_planned=..., bytes_planned=..., own_progress_count=0, report_own_progress=False)` → `create_sync_urls(file_list)` → `create_curl_download_instructions()` → `create_sync_folder_manifest_command("after-sync", back_ground=True)` → `create_check_checksum_instructions(...)`. `PrepareDownloadTempFiles` keeps `own_progress_count=0` (`total_progress_count` sums it even when progress is off, so any other value adds units to the "of N" total that are never incremented) but reports: it walks every download item, and with `report_own_progress=False` the whole pass was silent.
...
1. **`InstlInstanceSync_boto` will crash, not no-op** — `init_sync_vars` calls `os.fspath` but the module never imports `os` (`instlInstanceSync_boto.py`). A `BOTO` repo type raises `NameError` rather than silently doing nothing. Either remove the BOTO backend and the `case "BOTO"` in `do_sync` (`instlClientSync.py`), or add `import os` and implement real instructions.
2. **Dead S3/boto path** — `have_boto = False` is hardcoded (`connectionBase.py`); `ConnectionS3` can never be constructed and `connection_factory` always returns `ConnectionHTTP`. Delete the boto path or reinstate it behind a real optional-dependency import.
3. **`@abc.abstractmethod` on a concrete method** — `ConnectionHTTP.translate_url` (`connectionBase.py`) is decorated abstract yet has the production body. Drop the decorator from the subclass (keep it only on `ConnectionBase`).
4. **`verbatim` flag is always False** — `verbatim=source_url==['url']` (`instlInstanceSync_url.py`) compares a `str` to `['url']`; the verbatim-URL path is unreachable. Determine the intended source-flag check (likely on the file item) and fix; add a test.
5. **P4 command quoting bug** — `p4 sync "$(SYNC_BASE_URL)/<path>"$(REPO_REV)` (`instlInstanceSync_p4.py`) has misplaced quotes and no separator before the revision. Harmless while unmaintained, malformed if re-enabled. Fix quoting and add tests, or remove the backend.
6. **Global mutable singleton** — `ConnectionBase.repo_connection` (`connectionBase.py,216-225`) is process-wide, not thread-safe, and not resettable; cached sessions can leak across multi-repo runs/tests. Inject the connection through `instlObj`/config or add an explicit reset/lifecycle hook.
...
8. **Two divergent instruction paradigms** — URL uses typed `pybatch` objects + `sub_accum` context managers; SVN/P4 append raw `platform_helper` strings (`instlInstanceSync_svn.py`, `instlInstanceSync_p4.py`). Port SVN/P4 to pybatch or gate/remove them and document URL-only support.
9. **Dead/unused code in URL backend** — unused `import win32api` (`instlInstanceSync_url.py`) and the `if False:` mount-point sizing block (`318-323`, plus `total_sizes_by_mount_point`). Remove them.
10. **Duplicated config coercion helpers** — `_config_var_bool/_config_var_int/_config_var_list` (`instlInstanceSync_url.py`) duplicate typed-read logic that likely belongs on the `config_vars`/`ConfigVar` API. Promote and reuse.
...
- `InstlClientFactory(initial_vars, command) -> InstlClient` (`pyinstl/instlClient.py`): a `match command:` dispatcher that lazily imports and instantiates the right subclass. Called from `instl_main.py` for mode `client`. For `synccopy` it defines an ad-hoc class **inline**:
  ```python
...
Base class for sync/copy/synccopy/remove/uninstall/report. A single 660-line "god class" owning the whole pipeline plus DB-status mutation, target-folder bookkeeping, sync-location resolution, require.yaml I/O, binary-version scanning, YAML representation, and name resolution. `InstlClientFactory` and the inline `InstlClientSyncCopy` class live in the same module.
...
- **`command_output()`** — `write_batch_file(self.batch_accum)`, dump config_vars to `__WRITE_CONFIG_VARS_TO_FILE__`, and `run_batch_file()` iff `__RUN_BATCH__`.
- **`init_default_client_vars()`** — derive `SYNC_BASE_URL_MAIN_ITEM` from `SYNC_BASE_URL`; when `TARGET_OS != __CURRENT_OS__` recompute `TARGET_OS_NAMES`/`TARGET_OS_SECOND_NAME`; load `AUXILIARY_IIDS` (warns if absent); set `__MAIN_DRIVE_NAME__`.
- **`calculate_install_items()`** — `calculate_main_install_items()` then `calculate_all_install_items()`, then `lock_table("index_item_t")`/`("index_item_detail_t")` to freeze the DB against further inserts.
- **`calculate_main_install_items()`** — expand `MAIN_INSTALL_TARGETS`: split GUIDs from IIDs (`utils.separate_guids_from_iids`), resolve GUIDs→IIDs (`iids_from_guids`), `resolve_special_build_in_iids()`, re-resolve IIDs. Writes `__MAIN_INSTALL_IIDS__`, `__MAIN_UPDATE_IIDS__`, `__ORPHAN_INSTALL_TARGETS__`. Sets `self.update_mode = "__REPAIR_INSTALLED_ITEMS__" in main_install_targets`.
- **`calculate_all_install_items()`** — marks `install_status` columns: set ignored iids, mark `main`, recursively pull dependents (`get_recursive_dependencies`) as `depend`, mark `update` + its dependents, gather `main`+`depend` into `__FULL_LIST_OF_INSTALL_TARGETS__`; then `sort_all_items_by_target_folder(consider_direct_sync=True)` and `calc_iid_to_name_and_version()`. Status enum: `install_status = {"none":0,"main":1,"update":2,"depend":3}` (plus `remove` for uninstall).
- **`resolve_special_build_in_iids(iids)`** — expands `__REPAIR_INSTALLED_ITEMS__` / `__UPDATE_INSTALLED_ITEMS__` / `__ALL_GUIDS_IID__` / `__ALL_ITEMS_IID__` into their `depends`; repair takes precedence over update. Returns `(install_iids, update_iids)`.
- **`sort_all_items_by_target_folder(consider_direct_sync=True)`** — from `items_table.target_folders_to_items()` route each `(IID, folder, tag, direct_sync_indicator)` into `__no_copy_iids_by_sync_folder` (if direct-sync) or `__all_iids_by_target_folder` (else, `os.path.normpath`'d). Sets `__FULL_LIST_OF_DIRECT_SYNC_TARGETS__`; sorts each folder's iid list; then folds `source_folders_to_items_without_target_folders()` (sync-only sources, e.g. Icons) into `__no_copy_iids_by_sync_folder` under `$(LOCAL_REPO_SYNC_DIR)/...`.
- **`set_sync_locations_for_active_items()`** — per-source it computes per-file `download_path`/`download_root` and writes them via `info_map_table.update_downloads(items_to_update)`. Tag-dispatched (`!dir`/`!dir_cont`/`!file`), and branches on direct-sync vs copy. Key edge case: for direct-sync `!dir`/`!dir_cont` it short-circuits when an existing `Info.xml` on disk matches the source checksum (`utils.check_file_checksum`) — unless `update_mode` — calling `info_map_table.ignore_file_paths_of_dir()` to skip download. Maintains `ALL_SYNC_DIRS`.
- **`accumulate_unique_actions_for_active_iids(action_type, limit_to_iids=None) -> PythonBatchCommandBase`** — builds an `AnonymousAccum` of `EvalShellCommand` from de-duplicated action details (e.g. `pre_copy`, `post_remove`), with per-iid progress messages.
- **`accumulate_actions_for_iid(iid, detail_name)`** — same for per-iid action details like `pre_copy_item`/`post_remove_item` (not de-duplicated across iids).
- **require.yaml lifecycle**: `read_previous_requirements()` (chmods then reads `SITE_REQUIRE_FILE_PATH`, renames to `*.failed_to_read` on parse error); `create_require_file_instructions()` (emits Chmod + `CopyFileToFile` SITE→OLD, writes NEW from `repr_require_for_yaml()`, copies NEW→SITE, or `RmFile` if empty); `repr_require_for_yaml()` (aggregates `require_%` details per `owner_iid`, translating `require_version`→`version`/`require_guid`→`guid`, excluding auxiliary-iid and non-original-iid guid rows); `save_previous_state()` (debug copies `*_require_before.yaml`/`*_require_after.yaml`).
- **previous-source removal**: `create_remove_previous_sources_instructions_for_target_folder()` (only if folder exists, wraps in `Cd`); `_for_source()` (`!dir`→`RmDir`, `!file`→`RmFile`, `!dir_cont`→raises — illegal for previous_sources).
- **Name resolution**: `name_from_iid` (strips `_IID`, `_`→space), `name_and_version_for_iid`, `name_for_iid`.
- `create_sync_folder_manifest_command()` — one-shot `RunInThread(Ls(...))` writing `*-sync-folder-manifest.txt`, guarded by `SYNC_FOLDER_MANIFEST_FILE`.
...
Generates copy/unwtar instructions. `init_copy_vars()` sets `bytes_to_copy=0` and the **platform flags** `mac_current_and_target` / `win_current_and_target` (current OS *and* target OS both Mac/Win). Mutable state: `current_destination_folder`, `current_iid`, `bytes_to_copy`, `avoid_copy_markers`, `unwtar_batch_file_counter`.

- **`create_copy_instructions()`** — main driver:
  ...
  4a. the `copying` `ReportDownloadState` is appended only here, after steps 3–4: those contribute no bytes, so announcing the phase before them left its bar at zero through thousands of `MakeDir`s. The per-folder removal below is interleaved with the copies and stays inside the phase — separating it would mean restructuring the loop;
...
- **`create_copy_instructions_for_source(source, ...)`** — tag dispatch on `source[1]`: `!dir`→`_for_dir`, `!file`→`_for_file`, `!dir_cont`→`_for_dir_cont`; unknown tag raises `ValueError`.
- **`create_copy_instructions_for_file()`** — single file → `CopyFileToDir(..., hard_links=use_hard_links)` (+ Mac `ChmodAndChown` unless `.symlink`); if wtarred (asserts all-or-none wtar) → `Unwtar(first_wtar_full_path, os.curdir)`.
- **`create_copy_instructions_for_dir_cont()`** — `CopyDirContentsToDir(..., preserve_dest_files=True)` for non-wtar items (+ Mac per-item `ChmodAndChown a+rw` recursive and exec-bit `Chmod`); wtar items → `Unwtar(source_path_abs, os.pardir)` (parent, to avoid `X/X` nesting — unique to `!dir_cont`).
- **`create_copy_instructions_for_dir()`** — `CopyDirToDir(..., delete_extraneous_files=True)`; Mac exec-bit `Chmod` per non-wtar executable; `Unwtar` if wtars; Mac `Chown` recursive on destination folder. Falls back to `_for_file` if `get_dir_item` returns None (dir was wtarred).
- `create_copy_instructions_for_dir_extended()` — uses `CopyBundle(..., unwtar=has_wtars)`; contains a **dead commented-out block**.
- **`create_copy_instructions_for_target_folder()`** — `CdStage("copy_to_folder", target_folder_path)`; accumulates `pre_copy_to_folder`; per IID computes `flags` (`no_hard_links` → `use_hard_links=False`, `dont_downgrade`); wraps each source in `ShouldCopySource(source_path_abs, target_folder_path, dont_downgrade=...)` guard with `pre_copy_item`/source/`post_copy_item`; sets `scs.skip_progress_count`; Mac counts symlinks and emits `ResolveSymlinkFilesInFolder` if any items copied; `post_copy_to_folder` only if accum `is_essential()`.
- **`create_copy_instructions_for_no_copy_folder()`** — for direct-sync / sync-only sources: `pre/post_copy_to_folder` + `pre/post_copy_item`; `Unwtar(sync_folder_name, os.curdir, no_artifacts=False)` if `count_wtar_items_of_dir>0`.
- `size_of_source_items(source_items)` — raw source bytes, the unit `bytes_to_copy` is accumulated in. It is the same unit the copy phase reports back — `RsyncClone.copy_file_to_file` reports a copied file's size and `Unwtar._report_archive_bytes` reports an archive's compressed size — so a wtar item must not be scaled by an expansion estimate here. `create_copy_instructions_for_dir_cont` accumulates for every source item, including a directory that holds only wtar items: it still emits an `Unwtar`, whose bytes would otherwise be reported against a plan that never counted them.

### `InstlClientRemove` (`pyinstl/instlClientRemove.py`)

- **`create_remove_instructions()`** — reads `HAVE_INFO_MAP_PATH` (falls back to `SITE_HAVE_INFO_MAP_PATH` if missing); section `'remove'`; walks target folders **reverse-sorted** (children before parents); `pre_remove` (over `__FULL_LIST_OF_INSTALL_TARGETS__`); per folder sets `__TARGET_DIR__`, emits `pre/post_remove_from_folder`, and per IID `pre/post_remove_item` around `create_remove_instructions_for_source`; finally `post_remove`.
- **`create_remove_instructions_for_source(IID, folder, source)`** — three cases on the item's `remove_item` detail:
...
Subclass of `InstlClientRemove`. Overrides `calculate_install_items()` to call `calculate_main_install_items()` + `calculate_all_uninstall_items_option_2()`.

- **`calculate_all_uninstall_items_option_2()`** — reference-counting over require-translate rows from `items_table.get_all_require_translate_items()`:
...
- `calculate_all_uninstall_items_option_1()` — near-identical older algorithm, **dead code** (not wired in).
- **`create_uninstall_instructions()`** — if any targets: `create_remove_instructions()` + `create_require_file_instructions()`, and injects into section `'begin'` a `PythonDoSomething('PythonBatchCommandBase.set_a_kwargs_default("ignore_all_errors", True)')` so uninstall does not abort on a single failing command (also tolerates pre-python-batch index.yaml).

### `InstlClientReport` (`pyinstl/instlClientReport.py`)

Read-only. Overrides `calculate_install_items()` to a **no-op**. Accumulates rows into `self.output_data` and overrides `command_output()` to serialize to `__MAIN_OUT_FILE__` (or stdout) per `OUTPUT_FORMAT`: `json` (`json.dumps`), `yaml` (`aYaml.writeAsYaml`), or `text` (comma-joined lines); suppressed when `__SILENT__`.
- `do_report_versions()` — `items_table.versions_report(report_only_installed=__REPORT_ONLY_INSTALLED__)`.
- `do_read_yaml()` — forces `OUTPUT_FORMAT=yaml`, `DEBUG_INDEX_DB=True`, calls `verify_actions()`, dumps config_vars (`!define`) + resolved index (`!index`).
...
- **`calculate_full_doit_order()`** — requires `MAIN_DOIT_ITEMS`; for each iid recurse `resolve_dependencies_for_iid` (post-order append into `full_doit_order: unique_list`, so dependencies precede dependents); orphan iids (not in DB) are warned, written to `__ORPHAN_DOIT_TARGETS__`, and removed; result → `__FULL_LIST_OF_DOIT_TARGETS__`.
- **`do_doit()`** — three sections in order: `pre_doit`, `doit`, `post_doit`; per iid `doit_for_iid()` emits `Stage` + `Remark` begin/end wrapping `EvalShellCommand`s.

### `InstlMisc` (`pyinstl/instlMisc.py`)

Separate `do_something` mode (extends `InstlInstanceBase`). `do_command()` sets up dynamic-progress vars (`__START/_TOTAL_DYNAMIC_PROGRESS__`, `PROGRESS_STACCATO_PERIOD`), dispatches via `getattr`, optionally times the command (`PRINT_COMMAND_TIME`). `dynamic_progress()` does staccato progress logging.
...
- `find_leaves(item_graph)` — **buggy/dead**: `item_graph.neighbors(node)` returns an always-truthy iterator, so `if not the_neighbors` never matches; no callers.
...
2. **Duplicated uninstall algorithms** (`instlClientUninstall.py` vs). `_option_1` is dead (~60 duplicated lines) and mutates `req_trans` `status` in place. Delete `_option_1` and factor the shared require-count/closure loop into one named method that treats require-translate rows as immutable input.
...
> **Layout.** `pyinstl/instlAdmin.py` is one 1481-line module holding `InstlAdmin` plus the free
> functions `start_redis_heartbeat_thread`/`smart_merge_dicts`/`dict_in_canonical_order`.

### Component breakdown

#### `class InstlAdmin(InstlInstanceBase)` — `pyinstl/instlAdmin.py`
...
Key attributes set in `__init__(self, initial_vars)`:
...
- Regex attributes compiled lazily by helpers: `compiled_forbidden_folder_regex`, `compiled_forbidden_file_regex` (in `compile_exclude_regexi`), `compiled_should_be_exec_regex` (in `do_fix_props`/`do_fix_perm`), and the wtar regex family (`compiled_folder_wtar_regex`, `compiled_folder_exclude_wtar_regex`, `compiled_file_wtar_regex`, `compiled_wtar_by_file_size_exclude_regex`, `already_wtarred_regex`, `min_file_size_to_wtar`) set up in `prepare_conditions_for_wtar`.

Dispatch / lifecycle methods:
- `do_command(self)` — calls `set_default_variables()` then `getattr(self, "do_"+self.fixed_command)()`. This dynamic dispatch is the contract: each CLI subcommand maps 1:1 to a `do_<command>` method.
- `set_default_variables(self)` → `read_config_files()`.
- `read_config_files(self, reset_previous=False)` — if `reset_previous`, resizes the `config_vars` stack back to the saved depth (used by the daemon's `reload-config-files`); pushes a new scope, resolves each `__CONFIG_FILE__` via `self.path_searcher.find_file`, reads each yaml, then `resolve_defined_paths()`.
- `get_default_out_file(self)` — derives `__MAIN_OUT_FILE__` as `$(__CONFIG_FILE__[0])-$(__MAIN_COMMAND__).$(BATCH_EXT)`.
- `get_revision_range(self)` — parses `REPO_REV` of form `min[:max]` (regex `(?P<min_rev>\d+)(:(?P<max_rev>\d+))?`); returns `(min_rev, max_rev)` as a half-open range (`max` is +1).
- `get_last_repo_rev(self)` — runs the `SVNLastRepoRev` pybatch command against `SVN_REPO_URL`, returns `int(__LAST_REPO_REV__)`.
...
- `do_fix_props(self)` — sets `PythonBatchCommandBase.ignore_progress = True`, runs `SVNPropList`+`SVNInfo` into temp files `svn-proplist-for-fix-props.txt` / `svn-info-for-fix-props.txt` in the work folder, ingests them via `SVNInfoReader(format='info'|'props')` into `info_map_table`. Then, for every item, emits `SVNDelProp("svn:"+extra_prop, ...)` for stray props and, by matching `(item.isExecutable(), should_be_exec(item))`, emits `SVNDelProp('svn:executable',...)` (True/False) or `SVNSetProp('svn:executable','yes',...)` (False/True). Other combinations are no-ops.
- `do_fix_perm(self)` — walks the staging folder (limited by `prepare_list_of_dirs_to_work_on`), emits `Unlock(...,recursive=True)`, and per file compares `is_file_exec()` (stat exec-bit) vs `should_file_be_exec()` (regex) to emit `Chmod(a+x)`/`Chmod(a-x)`; finally `Chmod(folder, "a+rw,+X", recursive=True)`. Forbidden files (matched by `compiled_forbidden_file_regex`) are deleted **immediately** with `os.unlink(item_path)` — a side effect inside a batch-building method (see refactor notes).
- `do_fix_symlinks(self)` — emits `CreateSymlinkFilesInFolder` per folder.
- Exec-bit predicates: `is_file_exec` (stat `S_IXUSR|S_IXGRP|S_IXOTH`), `should_file_be_exec` (`EXEC_PROP_REGEX`), `should_be_exec` (file-only).

#### Staging↔SVN sync cluster: `do_stage2svn` / `stage2svn_with_comparator` / `do_svn2stage`

- `do_stage2svn(self)` — builds `(stage,svn)` path pairs (honoring `__LIMIT_COMMAND_TO__`), `raise_if_forbidden_dir`, emits `Unlock` + `Cd(svn_folder)`, then for each pair builds a `filecmp.dircmp(stage, svn, ignore=[".svn", ".DS_Store", "Icon\015"])` and calls `stage2svn_with_comparator`. The literal `"Icon\015"` (carriage-return Icon file) is a Mac-specific quirk.
- `stage2svn_with_comparator(self, comparator)` — recursive over `comparator.subdirs`. Handles three diff sets:
  - `left_only` (stage-only): rejects symlinks (`InstlException`, "run instl fix-symlinks"); skips forbidden files; **wtar transition special-case** — a `*.wtar.aa` file is *not* added if a `*.wtar` (same path minus `.aa`) exists with identical `utils.get_wtar_total_checksum`, recording the bare name in `do_not_remove_items`; otherwise emits `CopyFileToDir` + `SVNAdd`. New dirs are recursively scanned for forbidden entries, then `CopyDirToDir` + `SVNAdd`.
...
- `do_svn2stage(self)` — `SVNCleanup` (if checkout exists), then per limit `SVNCheckout(depth="infinity")` + `CopyDirContentsToDir(..., delete_extraneous_files=True)`.

#### Wtar cluster: `prepare_conditions_for_wtar` / `should_wtar` / `do_wtar_staging_folder`

- `prepare_conditions_for_wtar(self)` — compiles `FOLDER_WTAR_REGEX`, `FOLDER_EXCLUDE_WTAR_REGEX` (defaults to `a^`, matches nothing), `FILE_WTAR_REGEX`, `MIN_FILE_SIZE_TO_WTAR`, `WTAR_BY_FILE_SIZE_EXCLUDE_REGEX` (defaults to `.+`), and `already_wtarred_regex = re.compile(r"wtar(\.\w\w)?$")`.
- `should_wtar(self, dir_item: Path)` — returns `(_should_wtar, _already_tarred)`. Folders: wtar if `FOLDER_WTAR_REGEX` matches and exclude doesn't. Files: wtar if `FILE_WTAR_REGEX` matches, else if size > `min_file_size_to_wtar` and not in size-exclude regex. **Wraps the whole body in `try/except Exception: pass`**, silently swallowing stat/IO errors.
- `do_wtar_staging_folder(self)` — emits `Unlock`, `RmGlob('**/.DS_Store')`, `RmGlob('**/*~*')`, then runs a BFS worklist over the staging tree. For each non-wtar, non-symlink item it removes stale wtar parts (`utils.find_wtarred_parts_of_original` → `RmFile`); if `should_wtar` says yes, emits `Wtar(item, split_threshold=min_file_size_to_wtar)` + `RmFileOrDir(item)`; otherwise recurses into directory children. `total_redundant_wtar_files` is dead (always 0).

#### Verification cluster: `do_verify_index` / `do_verify_repo` + helpers

- `do_verify_index(self)` — reads index yaml + `FULL_INFO_MAP_FILE_PATH`, then `verify_actions()` (base class) + `verify_index_to_repo()`.
- `do_verify_repo(self)` — reads `STAGING_FOLDER_INDEX`, `info_map_table.initialize_from_folder(STAGING_FOLDER)`, `activate_all_oses()`. Runs `verify_inheritance` + `verify_dependencies` **before** `resolve_inheritance` (ordering is a hard invariant — both read raw detail rows). If any problems, prints and raises `AssertionError`. Otherwise `resolve_inheritance()`, `verify_actions()`, `verify_index_to_repo()`.
- `verify_inheritance` / `verify_dependencies` — collect missing `inherit`/`depends` IIDs via `items_table.get_missing_iids_from_details(...)` into a `defaultdict(list)` keyed by IID.
...
- `do_depend(self)` — builds `installItemGraph.create_dependencies_graph`, computes per-IID `depends` (`self.needs`) and `needed_by` (`self.needed_by`); empty lists become `None` so yaml renders `~`. Writes via `aYaml.writeAsYaml`.

#### Release pipeline: `do_up2s3` / `up2s3_repo_rev`

- `do_up2s3(self)` → `up2s3_repo_rev(int(config_vars['TARGET_REPO_REV']), self.batch_accum)`.
- `up2s3_repo_rev(self, repo_rev, batch_accum)` — the core release algorithm (see below). Re-entered by the wait-daemon's spawned subprocess.

#### Lightweight variant: `do_up_short_index` / `up_short_index_repo_rev`

- `up_short_index_repo_rev(self, repo_rev, batch_accum)` — near-duplicate scaffolding of `up2s3_repo_rev` (same assert trio, Redis client, STATUS/EXCEPTION config-vars, try/except-reraise, finally-email). Skips checkout/sync; only emits `IndexYamlReader` → `ShortIndexYamlCreator` → optional `SetBaseRevision` → `CreateRepoRevFile`, then `aws s3 cp` of the short-index and repo-rev file.

#### Activation: `do_activate_repo_rev`
...
#### Redis daemon: `do_wait_on_action_trigger` + helpers

Long-running loop. Optionally starts `start_redis_heartbeat_thread`. `BRPOP`s `WAITING_LIST_REDIS_KEY` (timeout 30s), writes `IN_PROGRESS_REDIS_KEY` status throughout. Control words: `stop` (break), `ping` (incr `<key>:ping`), `reload-config-files` (`read_config_files(reset_previous=True)`). Otherwise parses `what_to_do:domain:major_version:repo_rev`, maps trigger word → command via the inline dict `{'upload':'up2s3','up2s3':'up2s3','activate':'activate-repo-rev','short-index':'up-short-index'}`, writes a per-trigger `!define` config yaml that `__include__`s the domain/major config + main config, and **spawns a fresh `instl` process** via `mp.get_context("spawn").Process(target=instl_own_main, args=([..., instl_command_name, "--config-file", ..., "--log", ..., "--db", ":file:", "--run"]))`, then `join()`s it. The whole trigger branch is wrapped in `try/except Exception: log` — an unknown trigger word's `KeyError` is logged and swallowed.
- `report_instl_info_to_redis(self, redis_instance)` — `hmset` version/path/python/os to `INSTL_INFO_REDIS_KEY`.
- `print_wait_on_action_trigger_info(...)` — usage banner, printed every 30th iteration.
- `get_work_folder(self)` — `UPLOAD_WORK_AREA / domain / major / hierarchy`, created via `MakeDir`.
- `send_email_from_template_file(self, path_to_template)` — `ResolveConfigVarsInFile` then `utils.send_email_from_template_file`; on failure appends the traceback to the resolved file rather than raising.

#### Manifest collection: `do_collect_manifests` + writers

Walks each `COLLECT_MANIFESTS_DIR`, recursing one level per top-level dir (the dir name becomes `top_level_tag`), reading every `*manifest.yaml`. Two classes are defined **inside the method**: `ManifestItem` (`@dataclass`) and `ManifestYamlReader(ConfigVarYamlReader)` which registers `manifest_node_reader` for `__no_tag__`, the generic map tag, and `!index`, collecting nodes (skipping `template*` keys) into `manifest_nodes[iid]`. Only IIDs ending in `_IID` are kept. Merge logic by count: 1 → tag `Common`; 2 → `dictdiffer.diff` (ignoring `top_level_tag`); identical → `Common`, different → `smart_merge_dicts` + `dict_in_canonical_order` → `Common`; >2 → reported and cleared. Results bucketed into `{Common, Mac, Win}` and written as yaml (`write_collected_manifests_yaml`,, with tags `!index`/`!index_Mac`/`!index_Win`, prepending `COLLECT_MANIFESTS_BASE_INDEX` verbatim) or json (`write_collected_manifests_json`,, flat list with `_children` for `depends`).

#### Misc utilities

- `do_file_sizes(self)` — emits `path, size` lines via `utils.excluded_walk` honoring forbidden regexes.
- `do_read_info_map(self)` — loads info-map files into `info_map_table`.
- `do_check_instl_folder_integrity(self)` — recomputes `utils.get_file_checksum` for `index.yaml`, `info_map.txt`, and each split info-map listed in the index, comparing to `INDEX_CHECKSUM`/`INFO_MAP_CHECKSUM`/per-item `checksum`.
- `do_translate_guids(self)` / `translate_guids_in_file` — annotates GUIDs (regex `[a-fA-F0-9]{8}(-[a-fA-F0-9]{4}){3}-[a-fA-F0-9]{12}`) with `# <IID>` from `items_table.get_all_iids_with_guids`, preserving atime/mtime. Per-file body in `try/except Exception: pass` and a `finally` `except: pass` unlink.
- `do_short_index(self)` — `IndexYamlReader` → `ShortIndexYamlCreator`.
- `do_dump_config_vars(self)` — dumps all config vars as a `!define` yaml doc, annotating raw vs resolved values.

#### Module-level functions

- `start_redis_heartbeat_thread(redis_host, redis_port, heartbeat_key, heartbeat_interval)` — daemon thread setting `heartbeat_key` to the current timestamp every interval; loop body wrapped in `try/except Exception: print`.
- `smart_merge_dicts(by_os_dict)` — merges per-OS dicts (Linux/Mac/Win), hoisting keys identical across all OSes to the top level and keeping differing keys under their OS bucket, using `dictdiffer.diff` to compare. Assumes inputs already differ.
- `dict_in_canonical_order(to_order, order=None, single_value=None)` — recursively reorders mapping keys to `order` (extras appended), and collapses single-element sequences for keys in `single_value`.

### Key algorithm: `up2s3_repo_rev` (release pipeline) —

1. **Guard asserts**: `repo_rev >= BASE_REPO_REV`, `repo_rev >= IGNORE_BELOW_REPO_REV`, `repo_rev not in IGNORE_SPECIFIC_REPO_REV`. These are bare `assert`s (disabled under `python -O`).
...
- **Platform-specific**: Mac `Icon\015` / `Icon\r` handling (`dircmp` ignore, immediate `os.unlink`); `.DS_Store`/`*~*` removal in wtar; the subsystem is restricted to Mac/Linux by `instl_main`.
...
1. **God-class** (`InstlAdmin`, whole file). ~1480 lines spanning ~10 unrelated command clusters sharing mutable instance state. Suggest splitting into collaborator classes/mixins (`RepoMaintenance`, `StageSync`, `Wtar`, `Verification`, `S3Upload`, `RedisDaemon`, `ManifestCollector`) or a command-object registry replacing `getattr("do_"+name)`.
2. **Duplicated upload scaffolding** (`up_short_index_repo_rev` vs `up2s3_repo_rev`). Identical assert trio, Redis setup, STATUS/EXCEPTION bookkeeping, and try/except/finally-email block. Extract a context manager `_repo_rev_upload_session(status_var, exception_var, email_template)` wrapping the lifecycle.
3. **config_vars used as a status/messaging channel** (`UP2S3_STATUS`/`UP2S3_EXCEPTION`, `UP_SHORT_INDEX_*`, `ACTIVATE_*`). Global mutable store doubles as inter-step state and email-template data. Suggest returning explicit result objects to the email renderer.
4. **Classes defined inside a method** (`ManifestItem` + `ManifestYamlReader`,/). Re-created each call, untestable. Hoist to module level (or a dedicated module).
5. **Silent exception swallowing**: `should_wtar` `try/except Exception: pass`, `do_translate_guids`, `is_file_in_s3`, heartbeat thread, and `write_collected_manifests_yaml` bare `except` around the base-index read. These hide real failures and can produce wrong artifacts silently. Catch specific exceptions and at least log; let unexpected ones propagate.
6. **Side effects in batch-building method** (`do_fix_perm` `os.unlink`). Move the Mac `Icon\r` quirk into a dedicated pybatch command so the method only emits batch ops.
7. **Dead/no-op code**: `skip_some_actions = False` threaded through pybatch calls; `total_redundant_wtar_files` always 0; commented-out `RmFile` guidance. Remove or wire to a real debug flag.
8. **Magic trigger→command dict inline in the daemon**. A `KeyError` on an unknown trigger is swallowed by the surrounding `except`. Define a single validated trigger registry.
9. **Hardcoded service clients**: `redis.Redis(...)` constructed in `up_short_index_repo_rev`, `up2s3_repo_rev`, `do_wait_on_action_trigger`, `do_activate_repo_rev` and `boto3.resource('s3')`. Untestable without live services; provide lazy cached accessors (`self._redis()`, `self._s3()`) or inject clients.
10. **Deprecated `hmset`** (`report_instl_info_to_redis`) — `hmset` is removed in redis-py 4+. Replace with `hset(key, mapping=...)`.

Relevant file: `pyinstl/instlAdmin.py`.
...
#### `CreateTkConfigClass(TkBase, convert_type_func)` — factory function
...
#### `FrameController` — base controller
...
#### `ClientFrameController(FrameController)`
...
#### `AdminFrameController(FrameController)`
...
#### `ActivateFrameController(FrameController)`
...
#### `admin_command_template_variables` — module dict
...
- **Single Tk root, created at import time** (`tk_global_master = Tk()`) and assigned to `InstlGui.master`. The mainloop runs from `do_command`.
- **`__internal_update` guard** is the key invariant of the binding (see algorithm 1).
- **`__SEARCH_PATHS__` must be cleared** before re-reading config YAMLs so stale `__include__` resolution paths are not reused (`AdminFrameController.read_admin_config_files` and `ActivateFrameController.read_activate_config_files`).
- **Platform branches:** font size by `getattr(os, "setsid", None)`; subprocess spawn uses `preexec_fn=os.setsid` on Unix vs plain `Popen` on Windows (4 call sites) — note `run_client` additionally pipes stdout on Windows only, an inconsistency; `sys.executable` prepended on Windows when not frozen; `open_file_for_edit` tries `os.startfile(...,'edit')` then falls back to `open` on `AttributeError`; Mac-only `osascript` raise-to-front.
...
1. **Module-level `Tk()` at import time**. Creating the Tk root as an import side effect makes the module non-importable in headless/test contexts and couples every consumer to a global window. *Direction:* construct Tk lazily inside `InstlGui.__init__`/`do_command` and pass it down.

2. **God base class `FrameController`** mixes UI layout, file dialogs, filesystem access, subprocess spawning, stderr parsing, and clipboard. *Direction:* extract a `CommandRunner` (Popen + stderr parse) and a `FileOps` helper, leaving `FrameController` a thin view-controller.
...
5. **Activate tab couples to `InstlGui.notebook` for its timer**. The controller reaches through `instl_obj` into a widget it does not own to schedule callbacks. *Direction:* schedule via `self.frame.after` or inject a scheduler abstraction.
...
7. **Dead/unused code and stale TODOs:** `ADMIN_GUI_LIMIT_values` computed then never used; `DOMAIN_REPO_TO_UPLOAD`/`REPO_REV_TO_UPLOAD` and the upload feature marked `#todo oren - ??`; commented-out `grid_columnconfigure` and log lines. *Direction:* delete the dead assignment and comments; resolve or ticket the upload TODOs (referenced as SI-300).
...
9. **Scattered OS/platform checks** with inconsistent sources of truth — `getattr(os, "setsid", None)` vs `'Win' in config_vars["__CURRENT_OS_NAMES__"]` vs `config_vars["__CURRENT_OS__"] == "Mac"`. *Direction:* centralize platform decisions in one place and reuse.
...
`CUrlHelper` (abstract base) is the command generator. It accumulates planned transfers and emits curl config files plus a `.parallel-run` plan.
...
- Class-level `cached_internal_parallel`: tri-state cache of curl internal-parallel support; `external_parallel_header_text` / `internal_parallel_header_text` config templates.

Value objects:
- `CurlConfigFile` (dataclass): `path`, write-fd `wfd`, `num_urls`.
- `CurlDownloadEntry` (dataclass): `url`, `final_path`, `output_path`, `size`, `resume_from_byte`, `conditional_headers`, `force_restart_from_zero`. `has_per_transfer_options()` (resume or conditional headers present), `is_resume_entry()` (`resume_from_byte > 0`), `fresh_restart_entry()` clones the entry with `force_restart_from_zero=True` for the exit-33 fallback.

Important methods:
- `add_download_url(url, path, verbatim=False, size=0, download_last=False, output_path=None, resume_from_byte=0, conditional_headers=())`: translates the URL via `connectionBase.connection_factory(config_vars).translate_url` (unless `verbatim`), validates `resume_from_byte >= 0` (raises `ValueError`), and rejects `\r`/`\n` in `conditional_headers`. Appends a `CurlDownloadEntry` to the normal or `_last` list. Called by `instlInstanceSync_url.py`.
- `is_internal_parallel_supported()`: lazily runs `curl --version`, parses with `re.search(r"curl\s+([0-9.]+)\s", ...)`, compares against `min_supported_parallel_curl_version = "7.66.0"` via `packaging.version.Version`, caches the result in the classvar. `use_internal_parallel()` ANDs that with `config_vars["PARALLEL_DOWNLOAD_METHOD"] == "internal"`.
- `_create_config_files_for_entries(...)`: writes per-file curl config headers and per-transfer sections (see Algorithms). Owns the offline-grace retry-line policy.
- `_partition_resume_entries(entries)`: splits into fresh vs resume.
- `create_download_instructions(dl_commands)`: top-level entry called from `instlInstanceSync_url.py`. Partitions fresh/resume × normal/last into phases, builds config files, and appends `ParallelRun` (external) or `CurlWithInternalParallel` (internal) commands plus `Progress`/`MakeDir`.
- `create_parallel_run_config_file(path, config_files)`: writes one `"$(DOWNLOAD_TOOL_PATH)" --config "<path>"` line per config file; `None` entries become a `wait` line. On Windows it normalizes the config-file path via `win32api.GetShortPathName`.
- `fix_path(in_some_path_to_fix)`: Windows-only 8.3 short-path workaround for curl's unicode-path handling; creates the parent dir (needed because `GetShortPathName` requires an existing path) and falls back to the long path on failure.
...
- `DownloadFailureClass` (str Enum) and `RETRYABLE_FAILURE_CLASSES` (frozenset): the single source of truth for retryability.
- `DownloadFailureInfo` (frozen dataclass): `failure_class`, `retryable`, `source`, `reason`, `curl_exit_code`, `http_status`, `retry_after_seconds`; `to_dict()` emits camelCase.
- `classify_curl_exit_code(exit_code, http_status=None, headers=None, received_bytes=None)`: maps curl exit codes (e.g. 5/6→DNS, 7→TCP, 28→timeout split by `received_bytes`, 18/33→`PARTIAL_TRANSFER`, 23→`DISK_WRITE`, 22→delegates to `classify_http_status`).
- `classify_http_status(status_code, headers=None)`: 429→`HTTP_429` (+Retry-After), 408→timeout, 5xx→`HTTP_5XX`, 401/403/407→`HTTP_AUTH_POLICY`, other 4xx→`HTTP_4XX`.
- `classify_exception(exc)`: walks the `__cause__`/`__context__` chain; delegates to `requests`/`urllib`/`OSError` classifiers, then falls back to message-text heuristics.
- `retry_after_seconds(header_value, now=None)`: parses an integer-seconds or RFC HTTP-date `Retry-After`.

#### `downloadRetry` (`pyinstl/downloadRetry.py`)

Data-driven retry policy and backoff.
- `RetryPolicy` (frozen dataclass): `max_attempts`, `base_delay_seconds`, `max_delay_seconds`, `jitter_fraction`, `restart_required`, `respect_retry_after`.
- `DEFAULT_RETRY_MATRIX`: `{DownloadFailureClass -> RetryPolicy}`; terminal classes use `RetryPolicy()` (`max_attempts == 0`).
- `RetryAction` (Enum: `RESUME`/`RESTART`/`FAIL_TERMINAL`) and `RetryDecision` (frozen dataclass) with `will_retry` and `to_event(...)` (the `download.retry_decision` payload).
- `compute_backoff_seconds(attempt, policy, retry_after_seconds=None, random_unit_fn=None)`: exponential `base * 2^(attempt-1)`, capped by `max_delay_seconds`, multiplied by full-jitter `(1 + jitter_fraction * unit)`, overridden upward by `Retry-After` when the policy honors it.
- `decide_retry(failure, previous_retry_count, *, resume_eligible=False, matrix=DEFAULT_RETRY_MATRIX, random_unit_fn=None) -> RetryDecision`.
- `sleep_backoff(delay_seconds, *, channel=None) -> bool`: delegates to `channel.sleep_or_wake` (defaulting to `get_global_channel()`), drains the consume-once `try_now` flag, falls back to `time.sleep` when no channel is available.
- `format_retry_decision_log_line(decision, ...)`: emits the legacy `DOWNLOAD_RETRY_DECISION <json>` line, dropping `_DISALLOWED_EVENT_FIELDS`.

Consumed by `info_mapBatchCommands.py/238/561`.

#### `downloadState` (`pyinstl/downloadState.py`)

Owns on-disk download state and the resume-eligibility decision.
- `DownloadStateStore`: rooted at `<bookkeeping_dir>/download-state`; `session_path()` → `session.json`, `journal_path()` → `files-journal.jsonl`. `from_bookkeeping_dir(...)`, `load/save_session`, `load_file`, `save_file`, `save_files(records, replace=False)`. Per-file state lives in one append-only journal of JSON lines, last entry per `fileId` winning: one file per item cost a write + flush + fsync + atomic replace each, and the pre-download pass writes one per download item. `save_files(..., replace=True)` rewrites and compacts it, and only `PrepareDownloadTempFiles` may — it is the one caller that enumerates the whole session. Unparseable lines are skipped on read, so a kill that cuts an append in half costs that one record. `file_path(file_id)` → `files/{file_id}.json` is the layout an instl before the journal wrote, still read as a fallback so an interrupted install stays resumable across the upgrade.
- `store_for_bookkeeping_dir(bookkeeping_dir)` / `reset_store_cache()`: one store per bookkeeping dir per process, so the journal is read once and every later lookup is a dict hit. `reset_store_cache` is the isolation hook for tests, alongside the other `reset_*` hooks the download singletons expose.
- Record dataclasses: `DownloadSessionRecord`, `DownloadFileRecord`, and sub-records `DownloadSourceState` / `DownloadTargetState` / `DownloadExpectedState` / `DownloadTransferState`; each with `from_dict`/`to_dict` and `schemaVersion` validation. Enums `DownloadSessionState` and `DownloadFileState`.
- `write_json_atomic(path, data)`: tmp file + `fsync` + `os.replace`, indent=2/sort_keys, with `finally` cleanup.
- `temp_path_for_final_path(final_path, file_id)`: `<final>.instl-<first 16 of file_id>.part`. `make_file_id(...)`: sha256 over `(revision, repo_path, checksum, size)`. `promote_verified_temp_file(temp, final, expected_checksum, actual_checksum=None)`: verifies sha1 then `os.replace` (raises `ValueError("bad checksum ...")` on mismatch). `get_file_sha1`/`checksum_matches`.
- `update_session_state(bookkeeping_dir, new_state, reason=None, *, session_id=None)`: best-effort persisted transition; swallows all errors and returns `None`.
- `resume_decision_for_download_item(file_item, source_url, bookkeeping_dir, resume_enabled=False, validated_hosts=(), validated_path_prefixes=(), require_conditional=True, signed_url_min_ttl_seconds=300) -> DownloadResumeDecision`: the resume gate (see Algorithms). Called by `instlInstanceSync_url.py` and `info_mapBatchCommands.py`.
...
| `DOWNLOAD_RECONCILE_MISSING_OUTPUTS` | `yes` | kill switch; `DOWNLOAD_RECONCILE_MAX_ROUNDS 3` |
...
| `DOWNLOAD_SESSION_POOL_SIZE` | `16` | connection-pool size for `DownloadManager`'s `requests.Session` (the redownload pass); headroom, not a concurrency setting — the pass is serial |
...
The actual curl execution engine invoked by the `curlHelper`-generated commands. `CurlWithInternalParallel` (`subprocessBatchCommands.py`) is only the pybatch command — 4 methods (`__init__`, `progress_msg_self`, `repr_own_args`, `__call__`) that carry the serialized numbers and hand them to a `CurlTransfer`, which owns all of the orchestration below.
- `ParallelRun._control_channel()`: a seam over `downloadTransfer.get_control_channel()` (lazy `get_global_channel()`, returning `None` if unavailable, so a non-curl `ParallelRun`, e.g. a copy, is never paused; the method stays as the hook tests override).
- `ParallelRun._run_with_pause_and_offline_hold(commands)`: the pause/offline-hold/network-retry/exit-33-fallback run loop (see Algorithms). `network_retry_budget = 12`.
- Shared by both drivers as module-level functions in `downloadTransfer`: `is_network_error(exit_code)` (checks `utils.NETWORK_ERROR_CURL_EXIT_CODES`), `can_run_fallback(fallback_config_file, exit_code, fallback_exit_codes)` (gates on a fallback config being set and the exit code being in `fallback_exit_codes`, `(33,)`), and `get_control_channel()`. `_run_fallback_after_curl_range_failure(exit_code)` exists on `ParallelRun` **and** on `CurlTransfer` and is deliberately *not* shared: one re-runs a parallel-run command file through `utils.run_processes_in_parallel`, the other hands curl a single `--config` file.
- **Connectivity-loss self-sufficiency (`CurlTransfer`, i.e. the internal-parallel path, only; a `TODO(external-parallel)` documents why `ParallelRun` keeps the older bounded-backoff behavior):**
  ...
  - `_reconcile_missing_outputs(...)` — after curl exits on **any** code, verifies every `output` path in the config exists on disk (curl `--parallel` can exit 0 while individual transfers failed permanently, and a non-zero exit means more outputs are missing, not fewer); takes the curl exit code as `curl_return_code` and keeps it as the verdict when nothing needed fixing, returning 0 once a round recovers every output; re-runs curl with a `.reconcile-NN` config containing only the missing entries (`_parse_curl_config_for_reconcile` / `_write_reconcile_config`), each rewritten by `_fresh_start_entry` (`continue-at = N` → `continue-at = -`, stale `If-*` conditional headers dropped — replaying a byte offset against a nonexistent output would silently corrupt the file). Up to `DOWNLOAD_RECONCILE_MAX_ROUNDS` rounds, each through `_run_config_with_recovery`; the checksum phase remains the final gate.
  ...
  - module-level `_client_handles_backend_hold()` plus `configVar`'s `config_var_bool` / `config_var_int` — defensive config reads; all new event emission is additionally gated on `DOWNLOAD_CLIENT_HANDLES_BACKEND_HOLD` (default off) so an old Central sees exactly the legacy event stream while recovery stays silent.

### Key algorithms

**1. Config-file writing + offline-grace policy** (`_create_config_files_for_entries`)
...
**2. Plan orchestration** (`create_download_instructions`): partition fresh/resume × normal/last; for the resume path build four phases (`-fresh`, `-resume`, `-fresh-last`, `-resume-last`), the resume phases also getting a `-fallback` config built from `fresh_restart_entry()` clones; emit `CurlWithInternalParallel` per config file (internal) or `ParallelRun` per phase (external), with `fallback_exit_codes=(33,)`.

**3. Resume gate** (`resume_decision_for_download_item`): sequentially fail closed when — resume disabled, no bookkeeping dir, endpoint host not in `validated_hosts`, `.part` size ≤ 0, partial ≥ expected size, sidecar unusable/missing, expected identity (file_id/size/checksum) mismatch, source URL/object-key mismatch, signed-URL TTL insufficient, or (when `require_conditional`) no conditional validator. Otherwise returns `can_resume=True` with `resume_from_byte = received_bytes` and `conditional_headers`.
...
- `files-journal.jsonl` — one `DownloadFileRecord` per line, last entry for a `fileId` winning; `source` (redacted URL, object key, etag, last-modified, content-length, version-id, signed-URL expiry), `target` (`finalPath`/`tempPath`/`sidecarPath`, the last pointing at the journal), `expected` (size/checksum/algorithm), `transfer` (state/receivedBytes/retryCount/lastFailureClass/lastUpdatedAt). `files/{file_id}.json` is the pre-journal layout, read as a fallback and never written.
...
- **Pause is cooperative.** The pause flag never interrupts curl mid-chunk; callers check `wait_if_paused()`/`sleep_or_wake()` between batches/sleeps, so `.part` and sidecar atomicity hold. `_run_event` is inverted (set == running).
...
7. **Duplicate Progress emission / leftover TODO.** In `create_download_instructions` the "Downloading with N processes in parallel" `Progress` is built and appended once before the internal/external branch, then built and appended again inside the `else` (external-parallel) arm — so an external-parallel run emits it twice. The same method still carries a `# TODO: Move class someplace else`. Remove the duplicate `Progress` block and resolve the TODO.
8. **Scattered Windows path branching.** `curlHelper` imports `win32api` at module top and again inside `fix_path`, with platform logic interleaved across `fix_path` and `create_parallel_run_config_file`. Isolate Windows path normalization behind a single helper imported once.
...
#### 2.1 `PythonBatchCommandBase` (`pybatch/baseClasses.py`)
...
- `stage_stack: list` — push/pop on enter/exit; used to compute `major_stage_str()` / `stage` for error reports.
- `instance_counter`, `total_progress`, `running_progress` — global progress counters.
- `runtime_duration_by_progress: dict` — maps `runtime_progress_num` → seconds, consumed by `PatchPyBatchWithTimings`.
- `ignore_progress` — set True when commands run outside a python-batch file.
- `config_vars_for_repr` — set to the global `config_vars` only during `PythonBatchCommandAccum.__repr__()`, so `repr()` resolves configVar values.
- `essential`, `call__call__`, `is_context_manager`, `is_anonymous` — per-subclass flags (defaults at).
- `kwargs_defaults` — the declarative defaults dict (`own_progress_count`, `report_own_progress`, `ignore_all_errors`, `recursive`, `reply_config_var`, `prog_num`, `skip_action`, `suspend`, `skip_chmod`, `skip_chown`, `output_script`, …).

**Key methods:**
- `__init_subclass__(cls, essential=True, call__call__=True, is_context_manager=True, is_anonymous=False, kwargs_defaults=None, **kwargs)` — called once per subclass declaration. Sets the class flags and performs a **copy-on-write merge** of `kwargs_defaults`: it copies the parent's dict into a fresh dict before applying the subclass overrides, so a subclass cannot mutate the parent's defaults.
- `__init__(self, **kwargs)` (abstract) — seeds every `kwargs_defaults` key as an instance attribute, then initializes `exceptions_to_ignore` (seeded with `SkipActionException`), `child_batch_commands`, `doing`, `_error_dict`, `current_working_dir`, and `non_representative__dict__keys`.
- `__enter__` — pushes self onto `stage_stack`, starts the timer, increments/outputs progress, captures cwd, calls the override hook `enter_self()`. On exception it routes to `__exit__`.
- `__exit__(exc_type, exc_val, exc_tb)` — exception policy: suppress if no exception or `ignore_all_errors`; else if `should_ignore__exit__exception()` matches, log and suppress; otherwise attach `raising_obj=self` to the exception so `PythonBatchRuntime.log_error` can find the originating command. Calls override hook `exit_self()`, records timing, honors `suspend`, warns on progress-count mismatch, pops the stage only when suppressing.
- `__call__(*args, **kwargs)` (abstract) — base raises `SkipActionException` when `skip_action` is set; subclasses **must** call `PythonBatchCommandBase.__call__(self, ...)` first, then do the real work.
- `__repr__` + `repr_own_args(all_args)` (override hook) + `repr_default_kwargs` + `named__init__param` / `unnamed__init__param` / `optional_named__init__param` — build the eval-able source. `optional_named__init__param` emits nothing when the value equals the default, keeping the script terse. All values are quoted/resolved via `utils.quoteme_raw_by_type(..., config_vars_for_repr)`.
- `add` / `__iadd__` / `sub_accum` — tree building. `add()` pattern-matches: a `PythonBatchCommandBase` whose `is_anonymous` is True has its children flattened into the parent; a `str` is converted via `pybatch.EvalShellCommand(instructions, "")`; anything iterable is recursed.
- `total_progress_count()`, `is_essential()`, `increment_progress()` — recursive progress accounting over `child_batch_commands`.
- `error_dict` / `error_dict_self` / `who_locks_file_error_dict` — assemble the JSON error report: instl/python version, `doing`, `major_stage`, dotted `stage` path, `instl_class` repr, representative `__dict__`, progress counter, cwd, OS description, optional minimal-version check, `CONFIG_VARS_FOR_ERROR_REPORT`, and exception/traceback fields. `who_locks_file_error_dict` is Windows-only (uses `__WHO_LOCKS_FILE_DLL_PATH__`) and is shaped to be passable to `shutil.rmtree(onerror=...)`.
- `representative_dict` / `__eq__` / `explain_diff` / `__hash__` — equality compares `__dict__` minus `non_representative__dict__keys`; `__hash__` hashes `tuple(sorted(self.__dict__.items()))`.
...
#### 2.2 `PythonBatchCommandAccum` (`pybatch/batchCommandAccum.py`)

Top-level accumulator; subclasses `PythonBatchCommandBase` but does no work (`__call__` is a no-op at).

- `section_order` — the ordered tuple of section names: `prepare, assign, begin, links, upload, pre, pre-sync, sync, post-sync, copy, post-copy, remove, admin, pre_doit, doit, post_doit, end, post, epilog`.
- `special_sections = ("assign", "epilog")` — rendered outside the `PythonBatchRuntime` wrapper.
- `sections: dict` — section name → `Stage` container.
- `set_current_section(section_name)` — validates against `section_order`, lazily creates the `Stage`.
- `add(child_commands)` — appends into the current section's `Stage`.
- `__repr__` — **the serializer** (algorithm below).
- `_python_opening_code` / `_python_closing_code` — emit the prolog (imports, `sys.path.append`, `utils.set_acting_ids`, `from pybatch import *`, `PythonBatchCommandBase.total_progress`/`running_progress` globals, logger config) and the closing `# eof`.
- `camel_to_snake_case` + nested `_create_unique_obj_name` — generate the `with … as <name>:` variable names.

#### 2.3 `RunProcessBase` (`pybatch/subprocessBatchCommands.py`)
...
- `get_run_args(run_args)` (abstract) — subclasses build the argv list.
- `__call__` — runs `subprocess.run(check=False, stderr=PIPE, shell=self.shell, bufsize=0)`; handles `detach` (Popen), `script`/`shell` single-string modes, `out_file`/`capture_stdout`. **stderr policy**: if the process wrote to stderr and `stderr_means_err` and exit code was 0, the code is forced to **123**; `ignore_all_errors` forces returncode 0. Calls `handle_completed_process()`.
- `should_ignore__exit__exception` — additionally ignores `CalledProcessError` whose returncode is in `ignore_specific_exit_codes`.

`CUrl` builds a single-file curl argv (`--fail --raw --silent --show-error --connect-timeout --max-time --retry --retry-delay -o trg src`).

#### 2.4 `CurlWithInternalParallel` (`pybatch/subprocessBatchCommands.py`) and `CurlTransfer` (`pyinstl/downloadTransfer.py`)

Runs the bulk download as a single `curl --config <file>` using curl's internal `--parallel`, parsing curl's progress lines and enforcing pause/resume + offline-retry through the control channel. While the offline hold is active, `_note_meter_files` FREEZES the files-done high-water it reports (and the legacy `Downloaded N of M files` line built from it): with the network down, curl burns through the queue with instant connection failures and its `Xfers` column counts a failed transfer as finished, so an unfrozen count climbs through the outage while zero bytes move — and that count is the sole driver of Waves Central's progress-bar percentage. The high-water catches up to curl's cumulative meter after the hold releases; the burned files are recovered by the verify/redownload pass. **Not** a `RunProcessBase` subclass — the transfer drives `Popen` directly because it needs to interleave progress parsing and pause checks.
...
- `CurlWithInternalParallel.__call__` — builds a `CurlTransfer` from those five values plus `fallback_config_file_path`/`fallback_exit_codes` and `progress_msg_self()` as the log `label` (so the transfer never reaches back into the command), calls `transfer.run()`, then `increment_progress()`.
- `bytes_to_string` / `string_to_bytes` — module-level in `downloadTransfer`; human-readable byte conversions; `string_to_bytes` is intentionally fail-safe (returns 0, never raises, so progress accounting can't break curl).
- `CurlTransfer.__init__` — initializes the cumulative progress carry-over state `_files_high_water`, `_bytes_baseline`, `_bytes_high_water`, `previously_downloaded_bytes` and the shared offline-hold budget `_hold_seconds_used`.
- `CurlTransfer.run()` — Windows long-path workaround via `win32api.GetShortPathName`; seeds `previously_downloaded_files` / `previously_downloaded_bytes` from the `files_delivered_so_far` / `bytes_delivered_so_far` class attributes when an earlier chunk in this process could measure itself (see the progress-base note below); obtains the pause channel via module-level `get_control_channel()`; then delegates to `_run_config_with_recovery` (the pause/offline-hold/backoff re-run loop) and, on **any** exit code, `_reconcile_missing_outputs` (re-download entries whose expected `output` file is missing — curl `--parallel` can exit 0 with permanently failed transfers); publishes its own true completion count via `_publish_delivered()` after each stage; finally runs the exit-33 fallback when `can_run_fallback(...)`. Returns curl's final code. See the Download Subsystem section for the full offline-hold / reconciliation / stall-watchdog method inventory and the `DOWNLOAD_CLIENT_HANDLES_BACKEND_HOLD` event gate.
- `CurlTransfer._run_curl_once(config_file_path_fixed, pause_check)` — one curl pass. `Popen([...curl, "--config", path], stdout=PIPE, stderr=STDOUT, start_new_session=True, bufsize=1)`. `start_new_session=True` makes curl a process-group leader so `terminate_process()`'s `os.killpg()` can stop it on pause. Parses each progress line with the verbose regex capturing `DL_percent, Dled, Xfers, Live, Speed, …`; only `Dled`/`Xfers`/`Live`/`Speed` are consumed. The `Total`/`Current`/`Left` time columns are **optional** in that regex because curl changed their empty representation: up to 8.19 an unknown column printed `--:--:--`, while 8.20+ leaves it literally blank. Requiring them meant no meter line at all parsed under the system curl on Windows (8.21), so the legacy progress line — still the sole driver of Waves Central's progress-bar percentage — was never emitted there. Returns `(returncode, paused)`.

#### 2.5 `CheckDownloadFolderChecksum` (`pybatch/info_mapBatchCommands.py`)
...
- `re_download_bad_files` / `_redownload_one_file` — pause-aware per-file bounded retry via `downloadControlChannel` and `downloadRetry`. With the flag on, the pass is bounded by `_RedownloadBudget` (`DOWNLOAD_REDOWNLOAD_MAX_TOTAL_BYTES` / `DOWNLOAD_REDOWNLOAD_MAX_SECONDS`, both default 0 = unlimited — the legacy pass always completed every attempted file, so a non-zero default would abandon slow-network recoveries that used to succeed) instead of a count cliff: the budget is checked **between** files (a file in flight is never abandoned), files beyond an exhausted budget stay counted as bad, and the seconds budget measures **active** time only — `_PauseTrackingChannel` (a delegating wrapper around the control channel) measures time blocked in `wait_if_paused` so an outage hold never burns the recovery budget. Each per-file progress message carries its `N of M` position: the pass reports with `increment_by=0` (recovery work was never in the progress plan, so counting it would push the counter past its planned total), which without a position left Central showing one frozen `Progress N of M` for the whole pass. Tests: `pyinstl/test/test_redownloadBudget.py`.
- **Per-file cost of the redownload pass** (`pybatch/downloadBatchCommands.DownloadManager`, the `dler` the pass downloads through). Two defects made each recovered file cost ~1s in the field, and both are fixed: `__call__` used `with self.session`, and `requests.Session.__exit__` calls `close()`, so the connection pool was destroyed after every file (one TLS handshake each — the sync log shows one `Starting new HTTPS connection (1)` per file); and it called `MakeDir` twice per file, whose "already exists, just fix permissions" branch runs `FixAllPermissions` unconditionally — two child processes each on Windows (`attrib`, `icacls`), so four process spawns per file. The session is now used directly and closed once in `exit_self`, its pool sized by `DOWNLOAD_SESSION_POOL_SIZE` (default 16), and `_ensure_dir` drives `MakeDir` once per directory per session — the first file in a directory still goes through MakeDir in full. Measured on loopback HTTP over 300 files: 300 connections and 76.1 ms/file before, 1 connection and 12.0 ms/file after. Tests: `pybatch/test/test_downloadManagerReuse.py`.
...
- `If(condition, if_true=None, if_false=None)` — `__call__`: if `condition` is a `str` it is **`eval`'d**; the result is called if callable else coerced to bool; the matching branch (which may be a single object or an iterable) is run. Branch objects that are context managers get `own_progress_count=0` and run as `with obj as it: it()`. `if_true`/`if_false` are added to `non_representative__dict__keys` to avoid serialization errors.
- `IsFile` / `IsDir` / `IsSymlink` / `IsEq` / `IsNotEq` / `IsConfigVarEq` / `IsConfigVarNotEq` / `IsEnvironVarEq` / `IsEnvironVarNotEq` / `IsConfigVarDefined` — **plain `object` predicates** (not `PythonBatchCommandBase`), each implementing `__call__() -> bool`, `__repr__`, `__eq__`. `IsConfigVarEq` returns True for an undefined var with no default.
- `ForInConfigVar(list_var_name, target_var_name, call_str, resolve_indicator='@')` — `__call__` iterates the configVar list; per value it `push_scope_context()` + `push_resolve_indicator('@')`, resolves `call_str`, builds a command via `pybatch.EvalShellCommand`, sets `report_own_progress=False`, and runs it.

#### 2.7 Reporting / runtime wrappers (`pybatch/reportingBatchCommands.py`)

- `AnonymousAccum` (`is_anonymous=True`, `call__call__=False`, `is_context_manager=False`) — pure container; `add()` flattens its children into the parent.
- `Stage` (`call__call__=False`, `is_context_manager=True`) — section container; `stage_str()` feeds the error-report stage path; `__exit__` stores `__TIMING_<SECTION>_SEC__` configVar with `command_time_sec`.
- `PythonBatchRuntime(name)` (`call__call__=False`, `is_context_manager=True`) — wraps the whole run. `__exit__`: on exception calls `log_error`; logs total run time; pops the stage. `log_error` finds the originating command via `exc_val.raising_obj`, dumps `error_dict(...)` as JSON to the log, and calls `emit_failed_session_state(exc_type)`, which moves the structured state machine to `failed` with the exception's class name as `reason` — without it a failed install leaves the last phase reported as still running, which a consumer cannot tell apart from a hang. The emit is lazily imported and never raises.
- `Echo` / `Print` / `Remark` / `Progress` / `ConfigVarAssign` / `PythonDoSomething` / `EnvironVarAssign` — most have `own_progress_count=0`; `Echo`/`Remark`/`ConfigVarAssign` emit literal source rather than runtime actions. `EnvironVarAssign` subclasses `PythonDoSomething` to emit `os.environ[...] = ...` source.
- `PatchPyBatchWithTimings(out_file)` — `__call__` post-processes the emitted script, appending per-line timing comments from `runtime_duration_by_progress`, producing the `.timings.py` twin.

#### 2.8 Copy engine — `RsyncClone` (`pybatch/copyBatchCommands.py`)

Base copy engine mimicking rsync; parent of all `Copy*`/`Move*`/`Rename`/`CopyBundle`/`CopyGlobToDir`. Options documented in `options_doc_str`. Maintains **class-level mutable global state**: `__global_ignore_patterns`, `__global_no_hard_link_patterns`, `__global_avoid_copy_markers`, `__global_no_flags_patterns` with `add_global_*` classmethods. Per-instance it merges global + local patterns into `__all_*` lists. Tracks `statistics` (defaultdict), `last_src`/`last_dst`/`last_step` for `error_dict_self`, and `hard_links_failed` (latches once so it stops retrying hard links). `CUrl`'s sibling copy commands (`CopyDirToDir`, `CopyFileToFile`, etc.) are re-exported from `__init__.py`.

#### 2.9 Removal taxonomy (`pybatch/removeBatchCommands.py`)

`RmFile`, `RmDir`, `RmFileOrDir`, `RmGlob`, `RmGlobs`, `RemoveEmptyFolders`, `RmDirContents` — all default `resolve_path=True`, ignore `FileNotFoundError`, and retry once after a `FixAllPermissions`. `RmFile.__call__` is a two-attempt unlink: on first `PermissionError`, if the path is a dir it delegates to `RmDir`, otherwise runs `FixAllPermissions` and retries; second failure calls `who_locks_file_error_dict` and re-raises. On macOS with `output_script` set, removals append `rm -f` lines to a post-install shell script instead of removing immediately.

#### 2.10 Filesystem primitives (`pybatch/fileSystemBatchCommands.py`)

`MakeDir`, `Chmod`, `Chown`, `ChFlags`, `Cd`, `Touch`, `SplitFile`, `Glober`, `FixAllPermissions`, `Unlock`. Heavy `sys.platform` branching: `Chmod.symbolic_mode_re` differs darwin vs win32; `parse_symbolic_mode_mac` / `parse_symbolic_mode_win` (the latter manipulates win32security DACLs); `ChFlags.flags_dict` keyed by platform. `MakeDir` retries on `PermissionError`/`FileExistsError` and removes obstacles. `AdvisoryFileLock` (`call__call__=False`) is a print-debug stub commented out of `__init__.py`.

#### 2.11 Archive commands (`pybatch/wtarBatchCommands.py`)

`Wtar` creates split bzip2 PAX tars with a `total_checksum` pax header for idempotent skip; `Wtar.check_tarinfo` normalizes uid/gid/uname and adds per-file checksum+mtime pax headers. `Unwtar.unwtar_a_file` reads split parts via `utils.MultiFileReader`, compares `total_checksum`, and `RmDir`s before `extractall`. `Unwtar._partition_independent(jobs)` splits `(wtar_file_path, destination_folder)` jobs into those whose destinations cannot collide — safe to extract concurrently — and the rest; it resolves each destination once, sorts by path components and compares neighbours, because sorting puts every descendant of a folder directly after it, so that finds the same overlaps an all-pairs comparison would without being quadratic in the job count. `Wzip`/`Unwzip` are zlib single-file; `ZipFlat`/`UnZip` are stdlib zip.
...
- `pybatch/POSIXBatchCommands.py` / `MacOnlyBatchCommands.py` / `WinOnlyBatchCommands.py` — POSIX `CreateSymlink`/`RmSymlink`/`SymlinkToSymlinkFile`/`SymlinkFileToSymlink` (cloud-safe symlink round-tripping); Mac `MacDock`; Win `WinShortcut`, `*Registry*`, `ResHacker*`, `FullACLForEveryone`. On non-Windows, the Win classes are aliased to `PythonBatchCommandDummy` stubs (`__init__.py`).
- `pybatch/new_batchCommands.py` — `CopyDirToDirEx` (alternative copy composing `CopyDirToDir + Chown + Chmod + Unlock`); wildcard-imported by `__init__.py`.

#### 2.13 `EvalShellCommand` (`pybatch/__init__.py`)
...
#### 3.1 Script serialization — `PythonBatchCommandAccum.__repr__` (`batchCommandAccum.py`)

1. Set `PythonBatchCommandBase.config_vars_for_repr = config_vars` so nested `repr()`s resolve configVars.
2. Append `PatchPyBatchWithTimings(__MAIN_OUT_FILE__)` to the `epilog` section.
3. Recompute `total_progress` = sum of each section's `total_progress_count()` + 1 for the `PythonBatchRuntime`.
...
7. Resolve configVars in the main body and replace unresolved with native-var patterns; concatenate prolog + body + epilog.
8. Reset `config_vars_for_repr = None`.

`_repr_helper(batch_items, io_str, indent)` recurses lists, then for a single node: increments `running_progress_count` by `own_progress_count`, assigns `node.prog_num`, and emits source by matching `(call__call__, is_context_manager)`:
...
#### 3.2 Bulk-download re-run loop (`CurlTransfer.run` → `_run_config_with_recovery`, `pyinstl/downloadTransfer.py`)
...
previously_downloaded_files = CurlTransfer.files_delivered_so_far or planned base    # measured, not planned
previously_downloaded_bytes = CurlTransfer.bytes_delivered_so_far or 0               # measured, or start at zero
curl_return_code = _run_config_with_recovery(config, pause_check, channel)
_publish_delivered()
return_code = _reconcile_missing_outputs(pause_check, channel, curl_return_code)   # EVERY exit code, not just 0
_publish_delivered()
if can_run_fallback(fallback_config_file_path, return_code, fallback_exit_codes): _run_fallback_after_curl_range_failure(return_code)
increment_progress()   # CurlWithInternalParallel logs an error first when return_code != 0
...
`_reconcile_missing_outputs` loops up to `DOWNLOAD_RECONCILE_MAX_ROUNDS` (default 3): parse the original config into header + per-download entries; entries whose `output` path is missing on disk are rewritten as fresh-start (`continue-at = -`, stale `If-*` headers dropped) into a `.reconcile-NN` config that re-runs through `_run_config_with_recovery`. It runs after **every** curl exit, not only exit 0. Terminal behavior is unchanged — after rounds/holds are exhausted it logs and continues; the checksum phase remains the final gate. A config that yields no parsable entries now warns rather than returning silently, because "nothing to reconcile" and "cannot read the config" were indistinguishable. All the new session_state/retry_decision emission is gated on `DOWNLOAD_CLIENT_HANDLES_BACKEND_HOLD` (default off).

**Cross-chunk progress base.** `curlHelper._add_internal_parallel_download_commands` seeds each chunk's `previously_downloaded_files` with the cumulative **planned** url count of the chunks before it. When a chunk dies early that base is a fiction: a chunk that delivered 932 of its 24,893 files still handed the next chunk a base of 24,893, so the legacy progress line — still the sole driver of Central's bar — read `Downloaded 25783 of 25783 files, Downloaded 0 of 6.60G` while 256MB of 7.09GB had transferred. `CurlTransfer` therefore keeps two process-global running totals of what the chunks have actually put on disk, both class attributes published by `_publish_delivered` at every stage boundary and seeded into the instance at the top of `run()`:

- `files_delivered_so_far` — from `files_actually_downloaded()`, which counts the config's `output` paths that exist. A chunk that cannot read its own config publishes nothing and the next chunk falls back to the planned base.
- `bytes_delivered_so_far` — from `_sum_this_chunk_part_bytes()`, which sums the same paths' sizes and returns `None` (publishing nothing) when the config could not be read. Both channels report bytes against the **global** `__NUM_BYTES_TO_DOWNLOAD__`, but the byte counters (`_bytes_baseline`, `_bytes_high_water`, and the `.part` sum) are per-instance and one instance is one chunk — so without this base every chunk boundary restarted the reported bytes at zero. With the default `PARALLEL_DOWNLOAD_METHOD=internal` there is always at least one boundary (the `download_last` / `Info.xml` chunk), so the bar collapsed from near-full to near-zero at the end of every install; with the resume path active there are up to four config-file groups, and with external parallel up to `PARALLEL_SYNC`.

Neither is ever cleared: one instl invocation runs one download phase, so the class outlives every chunk of it. A second phase in the same process would have to reset them first.

**Progress carry-over invariant** (`CurlTransfer._run_curl_once`): because each re-run re-launches curl (Xfers/Dled restart at 0):
...
- **Bytes**: cumulative — with curl `continue-at`, a run's `Dled` is only the *new* bytes, so `cumulative = previously_downloaded_bytes + _bytes_baseline + current_dled`, clamped via `_bytes_high_water`, capped at total. When the run ends/pauses, the run's final `Dled` is folded into `_bytes_baseline`; `previously_downloaded_bytes` is what the earlier chunks delivered (see the cross-chunk progress base above) and is constant for the chunk.

`ParallelRun._run_with_pause_and_offline_hold` (`subprocessBatchCommands.py`) implements a near-identical loop for the multi-process parallel runner; the pause check is only wired when the commands are curl (`downloadTransfer.is_curl_command`) so a pause can't affect e.g. a copy parallel-run.

#### 3.3 `kwargs_defaults` copy-on-write merge (`__init_subclass__`)
...
- **Download temp files** and the **resume sidecar journal** (downloadState, written only when `DOWNLOAD_RESUME_ENABLED`).
...
2. **Class-level mutable global state** (`baseClasses.py`) — `stage_stack`, `running_progress`, `total_progress`, etc. are process-global and mutated during enter/exit, which breaks `RunInThread` (subprocessBatchCommands.py) reentrancy. Move run-scoped state into an explicit run/session object (or thread-local) passed down the tree.
3. **repr→eval round-trip is security-sensitive and fragile** — `__init__.py` `EvalShellCommand` and `conditionalBatchCommands.py` `If.__call__` both `eval()` arbitrary strings from index.yaml; serialization requires `__init__`/`repr` to stay in perfect sync. Introduce a structured (dataclass/JSON) IR + registry-based deserializer; restrict `EvalShellCommand` to a whitelist of registered command names.
4. **Duplicated pause/offline/network-retry loop** — `ParallelRun._run_with_pause_and_offline_hold` (`subprocessBatchCommands.py`) and `CurlTransfer._run_config_with_recovery` (`pyinstl/downloadTransfer.py`) plus their same-named `_run_fallback_after_curl_range_failure` started as near-identical copies and have now diverged (the channel lookup and the `is_network_error`/`can_run_fallback` predicates are already shared, as module-level functions in `downloadTransfer`): the internal driver added offline-hold/probing, output reconciliation, and stall handling that the external one deliberately lacks (in-code `TODO(external-parallel)`). Extract a shared `CurlRunLoop`/`DownloadRunner` helper holding pause-check, backoff budget, and fallback logic.
5. **Download orchestration extracted from the batch command** — the resume/retry/telemetry/control-channel logic that `CheckDownloadFolderChecksum` (`info_mapBatchCommands.py`) used to embed now lives in `pyinstl/downloadVerify.py` as module-level functions (`resume_decision_for_file_item`, `emit_retry_decision`, `redownload_bad_files`, `redownload_one_file`, the `emit_*`/`precompute_*` verify helpers, `RedownloadBudget`, `PauseTrackingChannel`). The command is back to its 8 original methods and calls into that module. Remaining step: give `downloadVerify` a narrower, named interface instead of a flat function surface the command reaches into directly.
...
7. **Heavy platform branching inside single classes** — `fileSystemBatchCommands.py` `Chmod`/`Chown`/`ChFlags` interleave darwin/win32 logic in method bodies and class-body `if sys.platform`; `ExternalPythonExec.get_run_args` hardcodes `python3.12`/`py -3.12`. Extract per-OS strategy objects (`PermissionBackend`, `FlagBackend`) and move the import-time platform selection in `__init__.py` into an explicit registry.
8. **Confirmed latent bugs/typos**:
   - `baseClasses.py` `log_result` — the f-string is missing its leading `f`: `full_message += "; {exception_obj.__class__.__name__}: {exception_obj}"` emits literal braces. Add the `f` prefix.
   - `baseClasses.py` — `__hash__` over `tuple(sorted(self.__dict__.items()))` will raise on unhashable attribute values and includes non-representative keys; base it on `representative_dict` with hashable normalization, or drop `__hash__`.
   - `conditionalBatchCommands.py` — `IsSymlink` defines `repr_own_args` instead of `__repr__`; since it is a plain `object` (not a command), that method is never called, so `repr(IsSymlink(...))` falls back to the default object repr and breaks serialization of `If(IsSymlink(...))`. Give the `Is*` predicates a small common base with a real `__repr__`.
9. **Dead/disabled code** — `wtarBatchCommands.py` `can_skip_unwtar` (retVal forced False), `fileSystemBatchCommands.py` `AdvisoryFileLock` (print-debug stub, commented out of `__init__.py`), and the commented Dummy block in `new_batchCommands.py`. Remove or finish them.

Relevant files (all absolute):
- `pybatch/baseClasses.py`
- `pybatch/batchCommandAccum.py`
- `pybatch/__init__.py`
- `pybatch/subprocessBatchCommands.py`
- `pyinstl/downloadTransfer.py`
- `pybatch/conditionalBatchCommands.py`
- `pybatch/reportingBatchCommands.py`
- `pybatch/info_mapBatchCommands.py`
- `pybatch/copyBatchCommands.py`
- `pybatch/removeBatchCommands.py`
- `pybatch/fileSystemBatchCommands.py`
- `pybatch/wtarBatchCommands.py`
...
- `accessors.py` — typed, documented accessor helpers over the hottest read keys (OS identity, main input/output file, run-batch flag, repo-revs, instl version), the first `config_vars` containment seam (see ARCHITECTURE.md Theme 2).
...
> `config_vars[...]` hotspots remain a follow-up.
...
Module globals:
- `config_vars = ConfigVarStack()` — **the single global stack** imported by ~all subsystems.
...
1. **Process-wide mutable global singleton** — `configVar/configVarStack.py` (`config_vars = ConfigVarStack()`) and the `var_stack` alias in `configVar/__init__.py`. ~50 modules import and mutate this single object, making state implicit, test isolation hard (tests must `clear()`), and concurrency unsafe. *Direction:* inject the `ConfigVarStack` into constructors/commands, keeping the global as a thin default; generalize the existing `private_config_vars()` local-stack pattern.

2. **`eval()` on YAML-supplied conditions** — `configVar/configVarYamlReader.py`. Arbitrary code execution from config; `os`/`sys` are imported (lines 6–7) solely so `eval` can reach them. *Direction:* replace with a restricted/whitelisted expression evaluator (e.g. `ast`-based) and drop the `os`/`sys` imports.
...
4. **Three+ overlapping resolution implementations** — `resolve_str`, `resolve_str_to_list`, `shallow_resolve_str`, and `replace_unresolved_with_native_var_pattern`, plus the canonical parser path in `resolve_str_to_list_with_statistics`. The shallow/native-rewrite paths re-implement `$()` matching with their own regexes that can diverge from the `var_parse_imp` grammar. *Direction:* funnel everything through `var_parse_imp`; have `resolve_str`/`resolve_str_to_list` share one helper.

5. **Dead performance-statistics code** — `configVar/configVarStack.py, 253–265, 295–297, 408–410`. `resolve_time` is never written (timing commented out), so `print_statistics` permanently reports 0ms. *Direction:* remove the timing fields/`resolve_time`, or restore timing behind a debug flag.

6. **Debug-only stubs in production** — `configVar/configVarOne.py–46` (`value_is_set`/`value_is_get` no-ops with how-to-wire docstrings). *Direction:* move to a test/debug helper or remove.

7. **Inline self-test harness in the parser module** — `configVar/configVarParser.py–347` (`__main__` table) plus `resolve_variable_1`/`resolve_variable_2`/`parse_str` used only by it. *Direction:* migrate the parse cases into `configVar/test/testConfigVar.py` and drop the `__main__`/helpers from the importable module.
...
9. **`ConfigVar` hard-bound to the concrete owner's method names** — `configVar/configVarOne.py` (`set_callback_when_value_is_get` defaults to `owner.resolve_str`) and (`__iter__` calls `owner.resolve_str_to_list`). The value object cannot resolve without an owner exposing exactly these methods. *Direction:* define a narrow `Resolver` protocol (`resolve_str`, `resolve_str_to_list`) and depend on the abstraction.

10. **Redundant/dead constructor params in `ConfigVarYamlReader`** — `configVar/configVarYamlReader.py` then: `self.path_searcher` is set from the parameter then immediately overwritten with a new `SearchPaths(...)`, and `url_translator` is stored but unused. *Direction:* remove the redundant first assignment and unused params, or honor a passed-in `path_searcher` when provided.

11. **Vestigial `use_cache` parameter** — `push_scope_context(use_cache=True)` and `resolve_str_to_list_with_statistics`'s `use_cache=False` call site: caching was removed in 2.1.6.0 but the parameter remains and does nothing. *Direction:* drop the parameter.
...
- `open()` — connects, sets `row_factory = sqlite3.Row`, calls `configure_db()`. If the db is new (in-memory, or the file does not yet exist) it runs `create-tables.ddl`, `init-values.ddl`, `create-indexes.ddl` via `exec_script_file`. Idempotent: no-op if `__conn` already set. Both `SVNTable.__init__` and `IndexItemsTable.__init__` call `db.open()`.
- `configure_db()` — `PRAGMA foreign_keys = ON`, `PRAGMA user_version = 1`, sets `row_factory`, clears trace callback.
- `transaction(description=None, progress_callback=None, progress_callback_n_instructions=50*1024*1024)` — `@contextmanager` yielding `self.__curs`; calls `begin()` then `commit()` on success. `selection()` and `temp_transaction()` yield a *fresh* `self.__conn.cursor()` and do **not** commit (read-only / TEMP-table use). All three derive `description` from `inspect.stack()[2][3]` when not passed (with a `try/except IndexError` guard in `transaction`).
- `begin()/commit()/rollback()` — `begin()` first calls `commit()`, then `execute("begin")`, `transaction_depth += 1`; `commit()` does `__conn.commit()`, `transaction_depth -= 1`; `rollback()` resets depth to 0. All safety asserts are commented out.
- `exec_script_file(file_name)` — loads a `.ddl` from `ddl_files_dir` (or treats the name as a path if it is an existing file) and `executescript`s it inside a transaction.
- `select_and_fetchone(query, params=None)` / `select_and_fetchall(...)` — run a SELECT via `selection()`; flatten single-column results to plain Python values/lists; multi-column results are returned as `sqlite3.Row` lists. Re-raise `sqlite3.Error`.
- `create_function(func_name, num_params, func_ptr)` — registers a Python SQL function on the connection (used for `need_to_download_file`, `name_and_version`, direct-sync indicator).
- `lock_table(table_name)` / `unlock_table` / `unlock_all_tables` — install/drop three `BEFORE INSERT/UPDATE/DELETE` triggers per table that `raise(abort, ...)`, making the table read-only; tracks names in `locked_tables`. `IndexItemsTable.__del__` calls `unlock_all_tables()`.
- `set_progress_handler(cb, n)` + nested `ProgressCallBacker` — wires the sqlite progress callback (fired every *n* VM instructions) to a user progress function while inside a transaction/selection context.
- `close()` — closes the connection; if `PRINT_STATISTICS_DB` is set, prints `statistics`. `close_and_delete()` also imports `pybatch.RmFile` and deletes the on-disk db file.

#### `DBAccess` (descriptor) — `db/dbMaster.py`
Lazily creates the singleton `DBMaster` on first attribute access.
- `__get__` — calls `get_default_db_file()`, then builds `DBMaster(os.fspath(__MAIN_DB_FILE__), __INSTL_DEFAULTS_FOLDER__)` and caches it; also sets `__DATABASE_URL__`.
- `get_default_db_file()` — resolves `__MAIN_DB_FILE__` (defaulting to `:memory:`). Cases: `:memory:` → in-memory; `:file:` → choose a path next to `__MAIN_OUT_FILE__`, else `$(__MAIN_INPUT_FILE__)-$(__MAIN_COMMAND__)`, else the system Logs folder, appending `.$(DB_FILE_EXT)`; any other value → `utils.ExpandAndResolvePath`. If `_owner.refresh_db_file` is set and the file exists, it is removed via `utils.safe_remove_file`.
- `__delete__` — closes and clears the cached db.

#### `TableAccess` (descriptor) — `db/dbMaster.py`
...
#### `DBManager` (mixin / public entry point) — `db/dbMaster.py`
...
```
- `set_refresh_db_file(cls, to_refresh)` / `reset_db(cls)` — `reset_db` does `close_and_delete()` then re-creates the three descriptors and clears `refresh_db_file`; this is the escape hatch for tests/repeated runs against the shared singleton state.

#### `Statistic` — `db/dbMaster.py`
...
#### `SVNRow` — `svnTree/svnTable.py`
`__slots__` wrapper over an `svn_item_t` row (22 columns), constructed positionally in `__init__`.
...
- `__str__` renders the comma-separated `info_map.txt` line: `path, flags, revision[, checksum][, size if != -1][, url][, dl_path:'...'][, needed_for_iid]`. `str_specific_fields(fields)` renders only a subset (for dirs, only fields in `fields_relevant_to_dirs`).

#### `SVNTable` — `svnTree/svnTable.py`
All reads/writes/queries over `svn_item_t`. ~1600 lines. Constructed with the shared `DBMaster`; calls `db.open()`.
- **Readers**, dispatched by `read_func_by_format` (`info`/`text`/`props`/`file-sizes`) from `read_from_file(in_file, a_format="guess", ...)`. Skips files already in `files_read_list`.
  - `read_from_svn_info` — parses `svn info` blocks via regex, builds rows, bulk-inserts with `utils.iter_grouper(8192)`.
  - `read_from_text` — CSV parse of `info_map.txt`; derives `level/parent/leaf` (`level_parent_and_leaf_from_path`), `fileFlag`, `wtarFlag`/`unwtarred` (via `utils.wtar_file_re`), `symlinkFlag`.
  - `read_props` — appends `x`/`s` to `flags` (executable/special) and other svn props into `extra_props`.
  - `read_file_sizes` — bulk `UPDATE size WHERE path=...`.
- **Index management**: `create_indexes()` creates the unique `path` index, runs `update_parent_ids_q` (resolves `parent_id` by matching `parent` to a parent row's `path`), creates `parent_id` and `unwtarred` indexes, and sets `MIN_REPO_REV`/`MAX_REPO_REV` config vars. `reading_files_context()` drops indexes, yields, recreates.
- **Query helpers** returning `List[SVNRow]` via `SVNRowListToObjects`: `get_items`, `get_required_items`, `get_unrequired_items`, `get_download_items`, `get_file_items_of_dir`, `get_items_in_dir` (recursive vs `immediate_children_only`), `get_required_exec_items`, `get_any_item_recursive`. Counters: `num_items(item_filter)` (large `match` over filter names), `get_to_download_num_files_and_size`, `count_wtar_items_of_dir`.
- **Mutators**: `mark_required_for_file/dir/source/revision`, `mark_required_completion` (recursive UPDATE marking parent dirs of required files), `mark_need_download` (registers `need_to_download_file` SQL fn then sets `need_download=1` for required+non-ignored files whose checksum/path needs fetching, then recursively marks parent dirs), `clear_required`, `ignore_file_paths_of_dir`, `ignore_unrequired_where_parent_unrequired`, `update_downloads`, `set_base_revision`.
- **Cross-table SQL** (joins into the index tables — see coupling note): `mark_required_files_for_active_items` (`executescript` of 3 statements: mark sources of `install_status>0 AND ignore=0` items, mark their children, mark their parents), `populate_IIDToSVNItem` (fills `iid_to_svn_item_t`), `set_info_map_file`/`mark_items_required_by_infomap`, and `get_files_that_should_be_removed_from_sync_folder`.
- **URL resolution**: `get_sync_url_for_file_item(file_item)`; `repo_rev_to_folder_hierarchy` (`@lru_cache`); `get_sync_base_url_for_iid(iid, default_url)` (`@lru_cache`).
- **Writers**: `write_to_file`/`write_as_text` serialize rows back to `info_map.txt` in 8192-row batches.

#### `IndexItemsTable` — `db/indexItemTable.py`
...
- **YAML parsing**: `read_index_node` → `read_index_node_helper` (recurses into templates via `read_index_template_node`) → `item_from_index_node` → `read_item_details_from_node`. The detail reader handles `__if` conditionals (`eval_conditional`), OS-name sub-nodes, `actions`, `define`, and special-cases `install_sources` (absolute vs relative path; relative paths fan out into per-OS-group `Mac`/`Win`-prefixed rows), `depends` (resolves to a list), and lowercases `guid`. `read_index_node_one_by_one` is a debug-only full duplicate gated on `DEBUG_INDEX_DB`.
- **Require parsing**: `read_require_node`, `read_item_details_from_require_node`, `clean_require_items` — map previous→current IIDs.
- **Inheritance resolution**: `resolve_inheritance` → `prepare_inherit_order` (topological order via recursive `resolve_iid` plus a `check_inherit_order` assertion) → `get_resolve_item_query_for_iid` (INSERTs inherited details with `generation+1`, skipping `not_inherit_details`, only for active OSes). In non-debug mode all per-IID scripts are concatenated and run as one `executescript`, then the `owner_iid` index is created.
- **OS activation**: `activate_all_oses`, `activate_specific_oses(*for_oses)` (always adds `"common"`, `UPDATE active_operating_systems_t.os_is_active`), `reset_active_oses`, `get_active_oses`. The DDL trigger `adjust_active_os_for_details2` propagates `os_is_active` to every `index_item_detail_t` row on this UPDATE.
- **Detail queries** (the `get_*_details*` family, and): original vs resolved, active-only vs all-OS, with optional DISTINCT / `limit_to_iids`. E.g. `get_resolved_details_value_for_active_iid(iid, detail_name, unique_values=False)`, `get_sources_for_iid(the_iid)`.
- **Install-status state machine**: `change_status_of_iids(new_status, iid_list)`, `change_status_of_iids_to_another_status(old, new, iid_list, ...)`, `change_status_of_all_iids`, `get_iids_by_status`, `get_recursive_dependencies`. Updating `install_status` fires DDL triggers that synthesize `require_by`/`require_version`/`require_guid` details (and remove them on uninstall, status `< 0`).
- **GUID/IID translation**: `iids_from_guids(guid_list)` and `iids_from_iids` build a TEMP table to map.
- **Default items**: `create_default_items` and helpers synthesize `__ALL_ITEMS_IID__`, `__ALL_GUIDS_IID__`, `__REPAIR_INSTALLED_ITEMS__`, `__UPDATE_INSTALLED_ITEMS__`.
- **Binaries reconciliation**: `add_binary_versions`, `add_require_version_from_binaries`, `add_require_guid_from_binaries` — populate `found_installed_binaries_t`; DDL triggers `add_iid_to_FoundOnDiskItemRow_guid_*` back-fill the `iid` column by matching guid or filename against `install_sources`/`previous_sources`.
- **Misc**: `mark_direct_sync_items`, `set_name_and_version_for_active_iids`, `config_var_list_to_db`/`add_config_vars` (persist to `config_var_t` for reference only), `get_data_for_short_index`/`versions_report` (runs report views/`short-index.ddl`).
...
**4. Sync-folder diff (`get_files_that_should_be_removed_from_sync_folder`)**
...
### Refactoring notes (this subsystem)
1. **Broken transaction nesting & swallowed errors** — `db/dbMaster.py`. `begin()` calls `commit()` first, asserts are commented out, and `transaction()` swallows `OperationalError` (rollback then return as success). Use the `sqlite3` connection as a real context manager or explicit `SAVEPOINT`s, and re-raise (or return an explicit failure) on `OperationalError`.
...
4. **Cross-table coupling** — `svnTree/svnTable.py` `mark_required_files_for_active_items`, `populate_IIDToSVNItem`, `set_info_map_file`, `get_files_that_should_be_removed_from_sync_folder` all join `index_item_detail_t`/`iid_to_svn_item_t`, so the two "separate" tables are not independently usable and schema changes silently break `SVNTable`. Move these joins into a dedicated coordinator (e.g. `InstallPlanner`) depending on both tables.
5. **Duplicated detail-query boilerplate** — `db/indexItemTable.py` and. The `os_is_active`/`detail_name`/DISTINCT/`limit_to_iids` filters are copy-pasted ~10 times. Introduce one parametrized detail-query builder.
6. **Inconsistent IN-list quoting / SQL injection of status values** — `change_status_of_iids` and `change_status_of_iids_to_another_status__` build `'("a","b")'` by hand and f-string `install_status` values directly; `change_status_of_iids_to_another_status` uses bound `?`. Standardize on bound placeholders for IN-lists and never f-string status values.
7. **Dead / debug-duplicated code** — delete `change_status_of_iids_to_another_status__` (trailing `__`, unused), collapse `read_index_node_one_by_one` into a thin `DEBUG_INDEX_DB` wrapper instead of a full copy, and remove `get_items_for_default_infomap` (`svnTree/svnTable.py`, marked "maybe use this").
8. **`SVNRow` positional/incomplete dunders** — `svnTree/svnTable.py`. `__init__` unpacks 22 columns by index (coupled to `SELECT *` order); `__eq__` for the `tuple` branch stops at index 20 and omits `needed_for_iid`; `__repr__` is a broken f-string (literal `{self.checksum}` etc. — only the first line is interpolated). Build `SVNRow` from `sqlite3.Row` by column name (or a dataclass), fix `__repr__`, and derive `__eq__` from the full field tuple.
9. **Brittle `inspect.stack()` descriptions** — `db/dbMaster.py`. `transaction`/`selection`/`temp_transaction` infer `description` from stack frames (with an `IndexError` guard). Most call sites already pass a description; require it explicitly and drop the introspection.
10. **Module-level mutable globals & singleton-via-descriptor** — `db/dbMaster.py` (`force_disk_db`, `unique_name_to_disk_db`, both unused) and the class-level `db`/`info_map_table`/`items_table` descriptors. Remove the dead globals and consider making the DB/tables an injected dependency rather than shared class state.
...
#### 1.1 Node monkey-patching (`augmentedYaml.py`)
...
- `GetYamlType()` → `YAML_TYPE.{SCALAR,SEQUENCE,MAPPING}` enum (defined `augmentedYaml.py`).
...
- `__getitem__` — scalar accepts index 0/-1; mapping does a **linear key scan** (`get_mapping_item`) comparing `str(item[0].value) == str(key)`; sequence supports +/- indices.
...
Important methods:
- `read_yaml_file(file_path, *args, **kwargs)` — entry point. Pushes path onto `file_read_stack`, fetches bytes via `utils.read_file_or_url_utf8(...)` (which may checksum-gate/cache), wraps the text in `io.StringIO` (setting `.name` for error reporting), then dispatches on extension: `.json` → `read_json_from_stream`, otherwise `read_yaml_from_stream`. When the outermost file finishes (`len(self.file_read_stack) == 0`), drains `post_nodes`. Catches `FileNotFoundError`/`URLError`/`YAMLError` (honoring `ignore_if_not_exist`) and a broad `Exception`; both log the file history and re-raise.
- `read_yaml_from_stream(stream, ...)` — `yaml.compose_all(stream)` yields nodes; each is pushed on the `YamlNodeStack` and dispatched.
- `read_yaml_from_node(node, ...)` — runs `convert_standard_tags`, clears+rebuilds `specific_doc_readers`, resolves the reader; `_post`-tagged docs are appended to `post_nodes` rather than executed immediately.
- `get_read_function_for_doc(node)` → `(reader, is_post_tag, effective_tag)`. No tag → `"__no_tag__"`; tag ending in `_post` strips the suffix and sets `is_post_tag=True`; unknown tag → `"__unknown_tag__"`.
- `read_json_from_stream(stream, ...)` — loads a JSON dict directly into `config_vars`; each value coerced to a `list` of str/int (raises `TypeError` on non-str keys or unsupported value types).
- `convert_standard_tags(node)` (static) — recursively normalizes `tag:yaml.org,2002:null` / `python/none` to `value = None`.
- `init_specific_doc_readers()` — override hook; base registers `__no_tag__`/`__unknown_tag__` to `do_nothing_node_reader`.

`YamlNodeStack` is a contextmanager-callable push/pop stack of nodes so error messages can report the exact source `start_mark`.

#### 1.3 `writeAsYaml` and dump wrappers (`augmentedYaml.py`)
...
7. **Trailing-newline hack**: emits the final EOL only when `sys._getframe(0).f_code.co_name != sys._getframe(1).f_code.co_name` — i.e. detecting top-level vs recursive call by comparing call-frame function names. The code comment notes this "will not work" if recursed from an outside function of the same name.

- `YamlDumpWrap(value=None, tag="", comment="", sort_mappings=False, include_comments=True)` — wraps a value with tag/comment/sort. `writePrefix`/`writePostfix` emit tags+comments; `ReduceOneItemLists` collapses single-element sequences; `GetYamlType/isMapping/isSequence/isScalar` delegate to module helpers.
- `YamlDumpDocWrap(..., explicit_start=True, explicit_end=False, ...)` — adds document markers `---` (with optional tag/comment) on prefix and `...` on postfix, resetting the `Indentor`.
- `Indentor(indent_size)` — tracks `cur_indent`, `num_extra_chars`, and an `item_type_stack` of `'l'`/`'m'` markers. `__iadd__/__isub__` adjust depth; `lineSepAndIndent` writes newline + `indent_size*cur_indent` spaces; `write_extra_chars`/`fill_to_next_indent` handle inline padding.

`nodeToPy(a_node, order=None, single_value=None, preserve_tags=False)` converts a node tree to python `OrderedDict`/list, reordering mapping keys per `order` (unknown keys appended), collapsing single-item sequences named in `single_value`, and optionally preserving `!`-tags (prepended to strings or set as a `tag` attribute on collections). `nodeToYamlDumpWrap` does the inverse conversion to `YamlDumpWrap` trees.
...
- `read_file_or_url_utf8(in_file_or_url, config_vars, path_searcher=None, save_to_path=None, checksum=None, connection_obj=None)` — primary reader used by `YamlReader`. If `save_to_path` already has the expected `checksum` it short-circuits by **recursively calling itself** on the cached file (dropping `path_searcher`/`connection_obj`). Local files are resolved via `path_searcher.find_file` then `abspath` (Win) / `realpath` (other); URLs go through `connection_obj.get_session(...).get(..., timeout=(33.05, 180.05))`. Returns `(buffer, actual_file_path)`.
- `open_for_read_file_or_url` (class) — context manager. For URLs it builds a `urllib.request` opener (custom headers via `translate_url_callback`), wraps the open in `patch_verify_ssl`, and retries **12 times** with `time.sleep(1.0)` between attempts. `actual_path` property exposes the resolved path/url.
- `read_from_file_or_url(...)` — reads and optionally verifies an expected checksum (errors on empty content, on `encoding != None` while checksumming, or on mismatch).
- `download_and_cache_file_or_url(in_url, config_vars, cache_folder, translate_url_callback=None, expected_checksum=None)` — checksum-gated cache. Cached filename = checksum if known, else the URL leaf. Re-downloads when the file is missing or its checksum fails. No checksum forces a fresh download.
- `download_from_file_or_url(...)` — caches then places on target, decompressing `.wzip` via `zlib`.
- `patch_verify_ssl(verify_ssl)` (contextmanager) — when `verify_ssl` is falsey, swaps `ssl._create_default_https_context` for a permissive factory: loads certifi CAs only (avoids the Windows cert store, which can trigger OpenSSL 3.x ASN1 errors), sets `CERT_NONE`, `check_hostname=False`, `@SECLEVEL=0`, and `OP_LEGACY_SERVER_CONNECT`; restores the original on exit. No-op when `verify_ssl` is True.

File-ops helpers: `chown_chmod_on_fd/chown_chmod_on_path` honor module globals `global_acting_uid`/`global_acting_gid`, set via `set_active_user_or_group_config_var_callback` (config-var hook for `ACTING_UID`/`ACTING_GID`) or `set_acting_ids`. `utf8_open_for_read` retries once through `FixAllPermissions` on `PermissionError`. Also: `safe_remove_file/_folder/_file_system_object`, `smart_copy_file` (hardlink-then-copy), `excluded_walk`/`scandir_walk`, `find_split_files`/`find_wtarred_parts_of_original`, `ExpandAndResolvePath`, `who_locks_file` (Windows DLL), and `wait_for_break_file_to_be_removed`.

#### 2.2 misc_utils (`utils/misc_utils.py`)

- OS/arch: `get_current_os_names()` (returns a tuple like `('Mac', 'Mac10.x')`), `GetMacArch`, `Is64Mac/Is64Windows/Is32Windows`, `GetProgramFiles32/64`.
- Checksums: `get_buffer_checksum`, `check_buffer_checksum`, `check_file_checksum`, `get_file_checksum(path, follow_symlinks=True)`, `compare_files_by_checksum`, `need_to_download_file`, and `get_recursive_checksums(some_path, ignore=None)` — produces `{relative_path: sha1, ..., "total_checksum": sha1}`, where `total_checksum` is the sha1 of the sorted, concatenated list of all checksums **and** paths (so renames change it; order does not).
- wtar parsing: `wtar_file_re` (groups `base_name`/`wtar_extension`/`split_numerator`), `is_wtar_file`, `is_first_wtar_file` (true for unsplit or `.aa`; rejects `._`-prefixed phantom files), `original_name_from_wtar_name`, `original_names_from_wtars_names`.
- `get_wtar_total_checksum(wtar_file_path)` — appends `.aa` if needed, feeds `find_split_files` into a `MultiFileReader("br", ...)` and reads `tar.pax_headers["total_checksum"]`. Swallows all exceptions (returns `None`).
- Collections: `unique_list` (a `list` subclass maintaining a backing set for O(1) membership and dedup), `set_with_order`, `write_to_list`.
- Formatting/timing: `max_widths`, `format_by_width`, `timing`/`time_it` decorators, `Timer_CM`.
- Action breadcrumb: module-global `doing_stack = []` with `add_to_actions_stack(action)` / `get_latest_action_from_stack()`.
- `get_curl_err_msg(key)` lazily imports `downloadFailures`. JSON helpers `extra_json_serializer`/`JsonExtraTypesDecoder`. System log path via `get_system_log_folder_path/_file_path`.

#### 2.3 Parallel execution (`utils/parallel_run.py`)

Module globals `exit_val`, `aborted`, `paused`, `process_list` are **reset at the start of each run**. Sentinels: `PAUSED_EXIT_CODE = 9990` and `NETWORK_ERROR_CURL_EXIT_CODES = frozenset({6, 7, 18, 28, 35, 52, 55, 56})`.

`run_processes_in_parallel(commands, shell=False, do_enqueue_output=True, abort_file=None, pause_check=None)`:
...
`run_process(command, shell, do_enqueue_output=True, abort_file=None, pause_check=None)` polls in a loop:
...
`extract_binary_info(in_os, in_path)` dispatches on file extension via `extract_info_funcs_by_extension` (a Mac/Win dict of extension → handler, default `default_extract_info`) and returns `(path, version, guid)`. Handlers: `Mac_bundle/Mac_framework/Mac_dylib/Mac_pkg`, `Win_bundle/Win_aaxplugin/Win_file`, with `plugin_bundle`, `get_info_from_plugin`, `get_guid`, `get_wfi_version` helpers (Mac uses `otool`/`PlistBuddy`/`pkgutil`/`uuidgen`). `check_binaries_versions_in_folder(current_os, in_path, in_filter=lambda p: True)` walks a folder collecting versions, and `check_binaries_versions_filter_with_ignore_regexes` is a callable filter built from ignore regex lists. Used by `pybatch/info_mapBatchCommands.py`.

#### 2.6 `log_utils` (`utils/log_utils.py`)

`config_logger(argv=None, config_vars=None)` and `setup_file_logging(log_file_path, level=logging.DEBUG, rotate=True, config_vars=None)` configure the root logger at startup with stdout/stderr stream handlers, `SameLevelFilter`/`ParentLogFilter`, rotating or plain file handlers, and per-level formatters (`PerLevelFormatter`, `CustomLogFormatter`, `JsonLogFormatter`). `func_log_wrapper` decorates functions for debug-level entry/exit logging.

`BufferUntilErrorHandler` is a proxy handler that **buffers records below `flush_level` (default ERROR)** in a bounded `deque(maxlen=max_records)`; on the first ERROR+ it emits a synthetic warning about dropped records, flushes the buffer to its targets, forwards the triggering record, and then **latches open** (subsequent records pass through). Installed/removed via `set_buffer_non_errors_until_error`/`set_log_quiet_until_error` using module globals `_buffer_until_error_proxy`/`_buffer_until_error_original_handlers`. Note `teardown_file_logging` begins with an unconditional `return` — its body is dead code.

#### 2.7 `ls` (`utils/ls.py`)

`disk_item_listing(files_or_folder_to_list, ls_format='*', output_format='text')` and `single_disk_item_listing(the_path, ls_format="PuUgGRTf", root_folder=None, output_format="text")` produce manifests of files/folders/wtars. A format-character mini-language drives per-item collection (e.g. `R`=permissions, `I`=inode, `L`=nlinks, `U`/`u`=user, `g`/`G`=group, `T`=mtime, `f`=checksum, `p`/`P`=path) via a `match`-based dispatch. Output can be `text` (column-formatted via `format_by_width`), dicts, or json (`format_char_to_json_key`). Platform branches: `unix_folder_ls`/`unix_item_ls` vs `win_folder_ls`/`win_item_ls`; `wtar_ls_func`/`wtar_item_ls_func` list inside split wtars via `MultiFileReader`. Used by `pybatch/reportingBatchCommands.py` and `pyinstl`.
...
`utils/email_utils.py`: `send_email(subject, content, sender, recipients, smtp_server, smtp_port)` (STARTTLS SMTP) and `send_email_from_template_file(path_to_template)` which builds parameters by `eval()`-ing the template file contents.
...
1. **Import-time monkey-patching of PyYAML** (`augmentedYaml.py`, high impact): mutating `yaml.ScalarNode/SequenceNode/MappingNode/Node` process-wide is order-dependent and brittle across PyYAML upgrades. Prefer a thin adapter wrapping nodes, or explicit helper functions, instead of patching library classes.
2. **`writeAsYaml` frame-name trailing-newline hack** (`augmentedYaml.py`, medium): comparing `sys._getframe(0/1).f_code.co_name` to detect top level silently breaks when called from a same-named wrapper. Thread an explicit `is_top_level` flag through the recursion.
3. **O(n) mapping lookup + side-effecting iteration** (`augmentedYaml.py` `get_mapping_item`, and `iter_mapping`/`iter_sequence` which assign `item.value = None`): build a key→value dict once per mapping node and move null normalization entirely into the explicit `convert_standard_tags` pass.
4. **Module-global mutable run state** (`parallel_run.py`, `files.py`, `misc_utils.py`, high): `exit_val/aborted/paused/process_list`, `global_acting_uid/gid`, and `doing_stack` make the code non-reentrant/thread-unsafe and `killall_and_exit` calls `sys.exit` on the shared list. Encapsulate per-run state in a `ParallelRunner` object and pass uid/gid via a context object.
5. **`get_disk_free_space` references undefined `win32file`** (`files.py`): `win32file` is never imported, so this raises `NameError` on Windows. Add the import (guarded) or centralize OS dispatch.
6. **Pervasive bare `except: pass`** (`extract_info.py`, `files.py`, `misc_utils.py`): corrupt plists / wrong versions / failed reads vanish silently. Narrow exception types and log at debug/warning instead.
7. **Dead/disabled code**: `log_utils.teardown_file_logging` returns immediately, making its body unreachable; `multi_file.py` and `augmentedYaml.py` carry `__main__` test blocks with hard-coded personal absolute paths; `dockutil` has commented-out legacy plist code. Remove or move into the real test suite.
...
9. **`send_email_from_template_file` uses `eval()`** on file contents (`email_utils.py`): arbitrary code execution risk. Use `ast.literal_eval` or a structured (yaml/json) template format.
10. **Duplicated download/checksum logic** (`files.py` `read_file_or_url_utf8` recursion at vs `open_for_read_file_or_url`, plus `download_and_cache_file_or_url`/`need_to_download_file`): one path uses `requests`, the other `urllib`; the recursion also drops `path_searcher`/`connection_obj`. Consolidate fetch+checksum+cache into one function and delegate.

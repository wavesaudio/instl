...
- **Scope note:** the new recovery lives in `CurlTransfer` (`pyinstl/downloadTransfer.py`), the collaborator the `CurlWithInternalParallel` command delegates to (the shipped path); `ParallelRun` (external parallel) intentionally keeps the older bounded-backoff behavior — it only sees an aggregate exit code across many curl processes (in-code `TODO(external-parallel)`); its completeness gate remains the checksum phase.

...
| `ParallelRun` / `CurlWithInternalParallel` (in PyBatch) | The actual curl execution engine: pause/offline-hold loop, network-error backoff, and exit-33 fresh-restart fallback. |
...
### Design notes & constraints
- The pause/offline-hold/network-retry/exit-33-fallback loop is duplicated across the two curl drivers and can drift.

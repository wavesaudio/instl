...
> Central side (confirmed): instl runs under `run-process` and Central writes `{cmd:"pause"|"resume"|"try_now", sessionId}` one-line JSON to instl **stdin** to drive the control channel; `DOWNLOAD_EVENT <json>` (and the legacy `DOWNLOAD_RETRY_DECISION <json>`) lines are parsed by Central's progress handler. Pause is cooperative — it gates *between* curl batches and does not kill in-flight curl; the hard-cancel path remains `run-process --abort-file`. Offline-grace: instl now owns offline detection (probe + hold in `CurlWithInternalParallel`) and emits `paused, reason="offline_no_network"` session_states plus network-class per-probe `retry_decision`s — but **only** when the driving client declares `DOWNLOAD_CLIENT_HANDLES_BACKEND_HOLD: true` (a new Central treats them as informational and never answers with a stdin pause). The shipped default is off, so an old Central — whose client-side detection (`navigator.onLine` + a network-failure streak heuristic) would answer with a never-auto-resumed pause on Windows — sees exactly the legacy event stream while the engine recovers silently. See `docs/download-events.md` §3.6.

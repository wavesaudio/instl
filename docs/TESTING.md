...
**The contrast to internalize:** the download subsystem alone has 14 test files and 216 test methods. Of the god-classes that *drive the actual product*, only `InstlClient` has any dedicated coverage (11 characterization methods added on this branch); `InstlAdmin`, `InstlGui` and `InstlInstanceBase` still have **zero**.

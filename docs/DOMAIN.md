...
> **`__include__` map form (remote fetch-then-cache).** `read_include_node` (`pyinstl/instlInstanceBase.py`) accepts not just scalar/sequence paths but also a **mapping** with keys `url:`, optional `checksum:`, and optional `copy:`. It downloads the file from the (resolved) `url` into the aux cache (verifying `checksum` via `download_from_file_or_url`), reads it as YAML, then — if `copy:` is present — emits `MakeDir` + `CopyFileToFile` batch commands (in the `post` section) to cache the fetched file to each `copy` destination (skipping a destination whose existing checksum already matches). This is how the index itself is loaded in production. Note the simpler `ConfigVarYamlReader.read_include_node` (`configVar/configVarYamlReader.py`) handles only scalar/sequence; the map form is added by the `InstlInstanceBase` override.
>

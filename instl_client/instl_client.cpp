...
fs::path default_discovery_path()
{
    ...
    return fs::path(root) / "Library" / "Application Support" / "Waves Audio" / "instl" /
           "server.json";
#endif
}
...
Socket connect_to(std::string_view host, int port)
{
    ...
    const int status = getaddrinfo(null_terminated_host.c_str(), service.c_str(), &hints, &results);
    if (status != 0)
    {
...
Json rpc(const Endpoint& endpoint, std::string_view method, const Json& params)
{
    ...
    const auto status_parse =
        std::from_chars(status_text.data(), status_text.data() + status_text.size(), http_status);
...
void print_usage()
{
    std::cerr << "Usage: instl-client [--discovery PATH] COMMAND [ARGS]\n"
              << "Commands:\n"
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        if (command == "ping"sv)
        {
            rpc(endpoint, "system.ping"sv).dump(std::cout, jsonland::dump_style::pretty) << '\n';
        }
        else if (command == "list"sv)
        {
            ...
            for (const auto& job : jobs)
            {
                ...
                std::cout.write(listed_job_id.data(),
                                static_cast<std::streamsize>(listed_job_id.size()));
                std::cout << "  ";
                std::cout.write(listed_status.data(),
                                static_cast<std::streamsize>(listed_status.size()));
                std::cout << "  ";
                ...
                for (const auto& argument : arguments)
                {
                    ...
                    std::cout.write(argument_text.data(),
                                    static_cast<std::streamsize>(argument_text.size()));
                    std::cout << ' ';
                }
                std::cout << '\n';
            }
        ...
        else if (command == "status"sv || command == "abort"sv || command == "follow"sv)
        {
            ...
            {
                rpc(endpoint, "jobs.get"sv, job_id_params(job_id))
                        .dump(std::cout, jsonland::dump_style::pretty)
                    << '\n';
            }
